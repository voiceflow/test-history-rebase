		...
		return (
			...
			<React.Fragment>
				<Modal isOpen={this.state.preview} size="xl" toggle={() => this.setState({ preview: false })} className="light-canvas-modal">
					...
					<Button className="goback-btn position-absolute" onClick={() => this.setState({ preview: false })} style={{
						top: 320,
						left: -90
					}}/>
				</Modal>
				...
				<div className="text-center pt-5">
					<div className="module-modal-icon">{this.props.module &&
					<div className="module-card-icon"><ModuleIcon module={this.props.module}/></div>}</div>
					...
					<div className="row justify-content-center mb-3">
						<Button isWhite className={cn('mr-2', { disabled: this.state.loading })} onClick={() => {
							this.setState({ preview: true });
						}} disabled={this.state.loading}>Preview</Button>
						<Button isPrimary className={cn('ml-2', { disabled: this.state.loading })} onClick={this.checkFlowConflicts}
