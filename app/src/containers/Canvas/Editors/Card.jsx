...
class Card extends Component {
    ...
    render() {
        let type = this.state.node.extras.cardtype
        return (
            <React.Fragment>
                <label className="mt-0">Card Type</label>
                <ButtonGroup className="toggle-group mb-2">
                    <Button outline={type !== 'Simple'} onClick={()=>this.updateContent('cardtype', 'Simple')} disabled={type === 'Simple'}> Simple </Button>
                    <Button outline={type !== 'Standard'} onClick={()=>this.updateContent('cardtype', 'Standard')} disabled={type === 'Standard'}> Standard </Button>
                </ButtonGroup>
                <label>Title</label>
                <VariableInput
