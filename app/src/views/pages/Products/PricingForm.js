...
class PricingForm extends React.Component {
            ...
            <DropdownMenu>
              {_.map(subTypes, category => (
                <DropdownItem
                  key={category}
                  value={category}
                  onClick={this.props.handleChange("subType")}
                >
                  {category}
                </DropdownItem>
              ))}
    ...
    return(
      <div>
        ...
        <InputGroup>
          ...
          <AvInput className="form-bg mb-3 mr-2 w-50"
            name="units"
  ...
  render() {
    return(
      <div>
        <AvForm onValidSubmit={() => this.props.updateStage(2)}>
        <AvGroup>
        ...
        {
          this.props.purchaseType === 'CONSUMABLE' ?
            this.getConsumable():
            null
        }
        {
          this.props.purchaseType ==='SUBSCRIPTION' ?
          ...
          <DropdownMenu>
            {_.map(TAX_CATEGORY, category => (
              <DropdownItem
                key={category}
                value={category}
                onClick={this.props.handleChange("taxCategory")}
                style={{zIndex: 100}}
              >
      ...
      <div className="product-stage-button">
          ...
          <Button className="purple-btn ml-2" style={{fontSize: '15px'}}
            variant="contained"
            color="primary"
          >
            Continue
