import { Alert, Button, Modal, SectionV2, System, Text, toast } from '@voiceflow/ui';
...
const AddSeats = manager.create('BillingAddSeats', () => ({ api, type, opened, hidden, animated, closePrevented }) => {
  ...
  const onAddSeats = async () => {
    ...
    try {
      await updateSeats(organization.id, subscription.id, numSeats);
      ...
      <SectionV2.SimpleSection headerProps={{ topUnit: 0, bottomUnit: 2 }}>
        <SectionV2.Description secondary lineHeight="20px">
          {isPaidPlan || isOnProTrial ? (
            <>
              Your workspace currently has {numberOfSeats} Editor {pluralize('seat', numberOfSeats)}. Seats you add here are available immediately and
              are billed on a pro-rata basis until the next billing date on {nextBillingDate}.
            </>
          ) : (
            <>
              ...
              {numSeats > editorPlanSeatLimits ? (
                ...
                <SectionV2.Description>
                  ...
                  <Text color="#62778C" paddingLeft="3px">
                    per Editor, per {billingPeriodUnit}
                  </Text>
                </SectionV2.Description>
              )}
            </div>
