import type { IntentClassificationLLMSettings as IntentClassificationLLMSettingsType } from '@voiceflow/dtos';
...
export const IntentPreviewSettings: React.FC<IIntentPreviewSettings> = ({
  testID,
  settings,
  initialSettings,
  onSettingsChange,
}) => {
  const combineSettings = ({ params, promptWrapper }: IntentClassificationLLMSettingsType) => ({
    ...params,
    promptWrapper: promptWrapper?.content,
  });
  ...
  return (
    <PopperModalSettings testID={testID}>
      <PopperOverridesDivider
        value={combinedSettings}
        initialValues={combinedInitialSettings}
        onReset={() => onSettingsChange(initialSettings)}
      />

      <IntentClassificationLLMSettings
        settings={settings}
        onSettingsChange={(value) => onSettingsChange({ ...settings, ...value })}
      />
    </PopperModalSettings>
  );
};
