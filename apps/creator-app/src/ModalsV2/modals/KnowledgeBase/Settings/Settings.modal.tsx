...
import { confirmBoxStyles, systemPromptStyles } from './Settings.css';
...
export const Settings = manager.create<Props>(
  'KBSettings',
  () =>
    ({ api, type, opened, hidden, closePrevented, animated, initialSettings, loadSettings }) => {
      ...
      const save = async () => {
        ...
        if (getIsFeatureEnabled(Realtime.FeatureFlag.VERSIONED_KB_SETTINGS)) {
          await client.api.fetch
            .patch<BaseModels.Project.KnowledgeBaseSettings>(`/versions/${versionID}/knowledge-base/settings`, settings)
            .catch(() => {
        ...
        api.enableClose();
        api.close();
        toast.success('Saved', { isClosable: false });
      };
      ...
      const resetToDefault = () => {
        ...
        toast.success('Restored to default', { isClosable: false });
      };

      React.useEffect(() => {
        if (settings === null) {
          toast.error('Unable to fetch knowledge base settings', { isClosable: false });
        }
      }, [settings]);
        ...
        >
          ...
          <Box pt={12} px={24} pb={24} direction="column">
            ...
            {SYSTEM_PROMPT_MODELS.has(settings?.summarization.model || AIGPTModel.GPT_3_5_turbo) && (
              <SystemPrompt system={settings?.summarization.system} onChange={update('summarization')} className={systemPromptStyles} />
            )}
          </Box>
          ...
          <Modal.Footer>
              ...
              {({ onToggle }) => (
                <Box className={confirmBoxStyles} direction="column" gap={10}>
                  ...
                  <Box justify="space-between" gap={8}>
                    <Button label="No" size="medium" fullWidth variant="secondary" onClick={onToggle} disabled={closePrevented} />
                    <Button
                      label="Yes"
                      size="medium"
                      fullWidth
            ...
            <Modal.Footer.Button
              ...
              disabled={_isEqual(settings, initialSettings)}
            />
          </Modal.Footer>
        </Modal.Container>
      );
