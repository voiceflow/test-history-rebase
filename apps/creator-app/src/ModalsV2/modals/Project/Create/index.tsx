...
import type * as Assistant from '@/components/Assistant';
...
const Create = manager.create<{ listID?: string }>(
  'CreateProject',
  () =>
    ({ api, type, opened, listID, hidden, animated, closePrevented }) => {
      ...
      const onChangeMemberRole = (memberID: number, role: Assistant.Member['role']) => {
        stateAPI.update(({ members }) => ({
          members: members.map((member) => (member.creator_id === memberID ? { ...member, role } : member)),
        }));
      };
