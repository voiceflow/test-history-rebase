...
const NewVersion = manager.create<Props, Result>(
  'PublishNewVersion',
  () =>
    ({ api, type, opened, hidden, animated, message, closePrevented }) => {
      ...
      return (
        <Modal type={type} maxWidth={392} opened={opened} hidden={hidden} animated={animated} onExited={api.remove}>
          ...
          <Modal.Body>
            <Box mb={16}>
              {message ??
                `This action will upload a new version to ${platformConfig.name}. Confirm you want to continue.`}
            </Box>
