...
const Purchase = manager.create<ITokenPurchase>(
  'TokensPurchase',
  () =>
    ({ api, type, opened, hidden, animated, workspaceID }) => {
      const subscription = useSelector(Organization.chargebeeSubscriptionSelector);
      const activeWorkspaceID = useSelector(Session.activeWorkspaceIDSelector)!;

      const [option, setOption] = React.useState<Nullish<(typeof PURCHASE_OPTIONS)[number]>>(null);
      const [loading, setLoading] = React.useState(false);
      const chargebeeTokenURLEndpoint = createChargebeeTokenURLEndpoint(
