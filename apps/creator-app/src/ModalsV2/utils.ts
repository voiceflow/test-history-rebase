...
export const openError = (props: ErrorProps = {}) =>
  manager.open('error', 'Error', { props }).catch(Utils.functional.noop);
export const closeError = () => manager.close('error', 'Error', 'api');

export const openSuccess = (props: SuccessProps) =>
  manager.open('success', 'Success', { props }).catch(Utils.functional.noop);
export const closeSuccess = () => manager.close('success', 'Success', 'api');

export const openConfirm = (props: ConfirmProps) =>
  manager.open('confirm', 'Confirm', { props }).catch(Utils.functional.noop);
