...
import { IAIModelSelect } from './AIModelSelect.interface';
import { AIModelSelectItem } from './AIModelSelectItem.component';

export const AIModelSelect: React.FC<IAIModelSelect> = ({ label, value, testID = 'ai-model-select', disabled, onValueChange }) => {
  const modelConfig = AI_MODEL_CONFIG_MAP[value];

  return (
    <Dropdown label={label} value={modelConfig.name} disabled={disabled} prefixIconName={modelConfig.icon} testID={testID}>
      {({ onClose }) => (
        <Menu>
