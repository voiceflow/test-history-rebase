...
import { ACTION_CONTEXT, MOCK_STATE, NODE_DATA, NODE_ID, PORT, PROJECT_META, SCHEMA_VERSION } from '../creator.fixture';

const { createState, describeReducer, describeReverter } = createDuckTools(CreatorV2, MOCK_STATE);

describe('Ducks | Creator V2 - insertStep reducer', () => {
  describeReducer(Realtime.node.insertStep, ({ applyAction, normalizeContaining }) => {
    ...
    it('insert a new step into an existing block', () => {
      ...
      expect(result.nodes).toEqual(
        normalizeContaining([blockNode, { ...stepData, nodeID: stepID }], (node) => node.nodeID)
      );
    ...
    it('register all ports of an inserted step', () => {
      ...
      expect(result.ports).toEqual(
        normalizeContaining([PORT, { id: inPortID }, { id: dynamicPortID }, { id: builtInPortID }, { id: byKeyPortID }])
      );
