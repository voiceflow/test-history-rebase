...
describe('Ducks | Creator V2 - isolateSteps reducer', () => {
  describeReducer(Realtime.node.isolateSteps, ({ applyAction, normalizeContaining }) => {
      ...
      expect(result.nodes).toEqual(
        normalizeContaining(
          [
            NODE_DATA,
            stepNode,
            { type: Realtime.BlockType.COMBINED, blockColor: '', nodeID: parentNodeID, name: blockName },
          ],
          (node) => node.nodeID
        )
      );
        ...
        links: normalize([
          {
            ...
            target: { nodeID: targetParentNodeID, portID: targetPortID },
          },
        ]),
