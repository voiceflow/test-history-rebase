...
const baseAttachmentReducer = reducerWithInitialState<AttachmentState>(createEmpty())
  ...
  .caseWithAction(Actions.Attachment.PatchOneCard, (state, action) => patchOne(state, action.payload.id, patchWithUpdatedFields(action)))
  .caseWithAction(Actions.Attachment.PatchOneMedia, (state, action) => patchOne(state, action.payload.id, patchWithUpdatedFields(action)))
