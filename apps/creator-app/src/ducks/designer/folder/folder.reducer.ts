...
export const folderReducer = reducerWithInitialState<FolderState>(createEmpty())
  .case(Actions.Folder.AddOne, (state, { data }) => appendOne(state, data.id, data))
  .case(Actions.Folder.AddMany, (state, { data }) => appendMany(state, data))
  ...
  .caseWithAction(Actions.Folder.PatchOne, (state, action) =>
