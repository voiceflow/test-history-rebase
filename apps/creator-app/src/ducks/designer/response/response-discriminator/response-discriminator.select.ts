...
import {
  channelParamSelector,
  createDesignerCRUDSelectors,
  languageParamSelector,
  responseIDParamSelector,
...
export const idByChannelResponseIDMapByLanguage = createSelector(
  idByLanguageChannelResponseIDMap,
  languageParamSelector,
  (state, language) => (language ? state[language] ?? null : null)
);

export const idByResponseIDMapByLanguageChannel = createSelector(
  idByChannelResponseIDMapByLanguage,
  channelParamSelector,
  (state, channel) => (channel ? state?.[channel] ?? null : null)
);

export const idByMapByLanguageChannelResponseID = createSelector(
  idByResponseIDMapByLanguageChannel,
  responseIDParamSelector,
  (state, responseID) => (responseID ? state?.[responseID] ?? null : null)
);

export const oneByLanguageChannelResponseID = createSelector(idByMapByLanguageChannelResponseID, map, (id, map) =>
  id ? map[id] : null
);
...
export const getOneByLanguageChannelResponseID = createSelector(
  ...
  (map, idMap) =>
    ({
      ...
      language,
      responseID,
    }: Parameters<typeof responseIDParamSelector>[1] &
      Parameters<typeof languageParamSelector>[1] &
      Parameters<typeof channelParamSelector>[1]) => {
