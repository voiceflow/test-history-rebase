...
export const root = createSelector(
  [createDesignerSelector(STATE_KEY), isFeatureEnabledSelector],
  (rootState, isFeatureEnabled) =>
...
export const mapByName = createSelector(all, (variables) =>
  Object.fromEntries(variables.map((variable) => [variable.name, variable]))
);
