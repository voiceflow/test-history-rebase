 ...
 */
export const goToAssistantOverview = (versionID: string) =>
  goTo(generatePath(Path.PROJECT_ASSISTANT_OVERVIEW, { versionID }));
...
export const goToCMSKnowledgeBase = (versionID: string) =>
  goTo(`${generatePath(Path.CMS_KNOWLEDGE_BASE, { versionID })}`);
 ...
 * @deprecated remove when FeatureFlag.CMS_WORKFLOWS is released
 */
export const goToDomainCanvasTextMarkup = (options: GoToDomainCanvasBaseOptions) =>
  goTo(generatePath(Path.DOMAIN_CANVAS_TEXT_MARKUP, options));

/**
 * @deprecated remove when FeatureFlag.CMS_WORKFLOWS is released
 */
export const goToDomainCanvasCommenting = (options: GoToDomainCanvasBaseOptions) =>
  goTo(`${generatePath(Path.DOMAIN_CANVAS_COMMENTING, options)}`);
