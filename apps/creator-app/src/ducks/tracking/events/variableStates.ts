...
export const trackVariableStateCreated = createProjectEventTracker<{ diagramID: string | null }>(({ diagramID, ...eventInfo }) =>
...
export const trackVariableStateEdited = createProjectEventTracker<{ editedFields: string[] }>(({ editedFields, ...eventInfo }) =>
  client.analytics.track(createProjectEvent(EventName.VARIABLE_STATE_EDITED, { ...eventInfo, edited_fields: editedFields }))
);

export const trackVariableStateApplied = createProjectEventTracker<{ type: VariableStateAppliedType }>(({ type, ...eventInfo }) =>
