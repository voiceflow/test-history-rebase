...
export const trackWorkspace = createBaseEventTracker<{ workspace: Realtime.Workspace }>(
  ({ workspace, ...eventInfo }, _dispatch, getState) => {
...
export const trackWorkspaceDelete = createBaseEventTracker<{ workspace: Realtime.Workspace }>(
  ({ workspace, ...eventInfo }) => {
    client.analytics.track(
...
export const trackDashboardLinkClicked = createWorkspaceEventTracker<{ linkType: string }>(
  ({ linkType, ...eventInfo }) =>
    client.analytics.track(
      createWorkspaceEvent(EventName.DASHBOARD_LINK_CLICKED, { ...eventInfo, link_type: linkType })
    )
);

export const trackPlanChanged = createWorkspaceEventTracker<{
  newPlan: PlanType | PlanName;
  currentPlan: PlanType | PlanName;
}>(({ newPlan, currentPlan, ...eventInfo }) =>
  client.analytics.track(
    createWorkspaceEvent(EventName.PLAN_CHANGED, { ...eventInfo, new_plan: newPlan, current_plan: currentPlan })
  )
);

export const trackSeatChange = createWorkspaceEventTracker<{ reduced: boolean; scheduled: boolean }>(
  ({ reduced, scheduled, ...eventInfo }) =>
    client.analytics.track(
      createWorkspaceEvent(EventName.SEATS_CHANGE, {
        ...eventInfo,
        type: scheduled ? 'scheduled' : 'immediate',
        seats: reduced ? 'reduced' : 'added',
      })
    )
);

export const trackStartCourse = createWorkspaceEventTracker((eventInfo) => {
  client.analytics.track(createWorkspaceEvent(EventName.WORKSPACE_START_COURSE, { ...eventInfo }));
});
