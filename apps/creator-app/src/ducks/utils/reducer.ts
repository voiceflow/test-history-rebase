...
export type ImmerHandler<State, Payload> = (
  draft: Draft<State>,
  payload: Payload,
...
export const createRootReducer = <State>(initialState: State): ReducerBuilder<State> => {
  ...
  reducer.mimerCase = (actionCreator, handler) => {
    ...
    return reducer.caseWithAction(actionCreator, (state, action) =>
      produce(state, (draft) => handler(draft, action.payload, action))
    );
  };
  ...
  reducer.immerCases = (actionCreators, handler) => {
    ...
    return reducer.casesWithAction(actionCreators, (state, action) =>
      produce(state, (draft) => handler(draft, action.payload, action))
    );
  };
...
export interface CreateSimpleReducer<State, BasePayload = any> {
  ...
  ): [actionCreator: ActionCreator<Payload>, handler: ImmerHandler<State, Payload>];
}
...
export interface CreateReducer<State, BasePayload = any> extends CreateSimpleReducer<State, BasePayload> {
  <Payload extends BasePayload>(
    ...
    handler: ImmerHandler<State, Payload>
  ): [actionCreators: ActionCreator<Payload>[], handler: ImmerHandler<State, Payload>];
}

export interface CreateCombinedReducer<State, BasePayload = any> {
  <Payload extends BasePayload>(
    ...reducers: Array<[actionCreator: ActionCreator<Payload>, handler: ImmerHandler<State, Payload>]>
  ): [actionCreator: ActionCreator<Payload>, handler: ImmerHandler<State, Payload>];
}
...
const validateCombinedActions = <State>(
  reducers: Array<[actionCreator: ActionCreator<any>, handler: ImmerHandler<State, any>]>
) => {
  ...
  (...reducers) => [
    ...
    (state, payload, action) =>
      reducers.reduceRight(
        (draft, [_, handler]) => handler(draft === undefined ? state : draft, payload, action) as any,
        state
      ),
  ];
