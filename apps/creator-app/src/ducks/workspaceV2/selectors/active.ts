...
import { toLegacyWorkspaceMember } from '@/ducks/utils/toLegacyMember';
...
export const localOrganizationSelector = createSelector(
  [workspaceSelector, getOrganizationByIDSelector],
  (workspace, getOrganizationByID) => {
...
const findWorkspaceMembers = (organization: Organization | null, workspaceID: string | null) => {
  if (!organization || !workspaceID) return [];

  return (
    organization.members
      .filter((member) => member.scope === RoleScope.WORKSPACE && member.workspaceID === workspaceID)
      .map(toLegacyWorkspaceMember) ?? []
  );
};

export const getAllMembersByWorkspaceIDSelector = createSelector([localOrganizationSelector], (organization) => {
  return (workspaceID: string) => findWorkspaceMembers(organization, workspaceID);
});

export const getMembersByWorkspaceIDSelector = createSelector(
  [getAllMembersByWorkspaceIDSelector],
  (getAllMembersByWorkspaceID) => {
    return (workspaceID: string) =>
      getAllMembersByWorkspaceID(workspaceID).filter(Realtime.Utils.typeGuards.isWorkspaceMember);
  }
);
...
export const membersSelector = createSelector(
  [Session.activeWorkspaceIDSelector, localOrganizationSelector],
  (workspaceID, organization) => {
    const members = findWorkspaceMembers(organization, workspaceID).filter(Realtime.Utils.typeGuards.isWorkspaceMember);

    return Normal.normalize(members, (member) => String(member.creator_id));
  }
);
...
export const pendingMembersSelector = createSelector(
  [Session.activeWorkspaceIDSelector, localOrganizationSelector],
  (workspaceID, organization) => {
    const members = findWorkspaceMembers(organization, workspaceID).filter(
      Realtime.Utils.typeGuards.isWorkspacePendingMember
    );

    return Normal.normalize(members, (member) => member.email);
  }
);
