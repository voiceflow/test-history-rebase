...
export const useGetCMSResourcePath = (resourceType: CMSRoute) => {
  ...
  return (resourceID: string) => {
    ...
    if (folderPaths) return generatePath(`${Path.CMS_RESOURCE}/${folderPaths}/${resourceID}`, { versionID, resourceType });
...
export const useOpenCMSResourceDeleteConfirmModal = () => {
  ...
  return ({ size, label, onConfirm }: { size: number; label: string; onConfirm: () => Promise<void>; messageLabel?: string }) => {
