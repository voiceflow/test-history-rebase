...
const ActionsSubMenu = React.forwardRef<HTMLDivElement, ActionsSubMenuProps>(({ parentPath, parentParams, sourcePortID }, ref) => {
  const engine = React.useContext(EngineContext)!;

  const goToActions = usePersistFunction(
    ({ sourcePortID, sourceNodeID, actionNodeID }: { sourcePortID: string; sourceNodeID: string; actionNodeID: string }) => {
      engine.setActive(sourceNodeID, {
        nodeSubPath: generatePath(parentPath ? `${parentPath}/${ACTIONS_PATH}` : ACTIONS_PATH, { ...parentParams, sourcePortID, actionNodeID }),
      });
    }
  );
