...
import type { ConnectedStep } from '../../../types';
import { ChoiceItem } from './components';
import { useChoiceStep } from './hooks';

const ChoiceStep: ConnectedStep<Realtime.NodeData.Interaction, Realtime.NodeData.InteractionBuiltInPorts> = ({
  data,
  ports,
  engine,
  palette,
      ...
      <Section v2 withIcon>
        {choices.length > 1 || (choices.length === 1 && choices[0].label) ? (
          choices.map((item, index) => (
            <ChoiceItem {...item} key={item.key} index={index} palette={palette} onOpenEditor={onOpenEditor} />
          ))
        ) : (
