...
const Card: React.FC<CardProps> = ({ color, title, level = 0, expandedProp = false, isLoading = false, children }) => {
  const [expanded, onExpand, onCollapse] = useEnableDisable(false);

  return (
    <S.Container onMouseEnter={onExpand} onMouseLeave={onCollapse}>
      <S.TitleContainer color={color} isLoading={isLoading}>
        {title}
        {!isLoading && <ProgressBar width={40} background="rgba(255, 255, 255, 0.3)" color="rgba(255, 255, 255, 0.5)" height={2} progress={level} />}
      </S.TitleContainer>
