...
const createController = ({ debug = false, context }: { debug?: boolean; context?: { trace: Trace[] } } = {}) => {
  const engine = {
...
const expectSetTimeout = (controller: TraceController) => expect(controller['timeout']['delay']);

const expectAudioPlay = (controller: TraceController) => expect(controller['audio']?.['play']);

const expectAudioStop = (controller: TraceController) => expect(controller['audio']?.['stop']);

const expectMessage = <T extends Exclude<keyof MessageController, 'trackStartTime'>>(
  controller: TraceController,
  method: T,
  data: any
) => expect(controller['message'][method]).toBeCalledWith(data);

const expectUpdateStatus = (controller: TraceController) => expect(controller['props']['updateStatus']);

const expectSetError = (controller: TraceController) => {
  expect(controller['props']['updateStatus']).toBeCalledWith(PMStatus.ERROR);

  return expect(controller['props']['setError']);
};

const expectSetInteractions = (controller: TraceController, interactions: { name: string; request?: BaseRequest }[]) =>
  expect(controller['props']['setInteractions']).toBeCalledWith(interactions);

const expectEnterFlow = (controller: TraceController) => expect(controller['props']['enterDiagram']);

const expectFocusNode = (controller: TraceController) => expect(controller['props']['getEngine']()!['node']['center']);

const expectProcessTrace = (controller: TraceController) => expect(controller['processTrace']);

const expectProcessSingleTrace = (
  controller: TraceController,
...
describe('Prototype/PrototypeTool/Trace', () => {
  describe('next()', () => {
