...
import type { KnowledgeBaseSources } from '@/pages/Prototype/types';
...
const BaseMessage: React.FC<BaseMessageProps> = ({
  ...
  const uniqueKbSourceNames = Utils.array.unique(
    knowledgeBase?.filter((source) => source?.name).map((source) => source.name) || []
  );

  const logoCircleUrl = avatarURL || 'https://cdn.voiceflow.com/assets/logomark.png';

  return (
    <S.Container
      focused={focused}
      className={cn(ClassName.CHAT_DIALOG_MESSAGE, className)}
      rightAlign={rightAlign}
      {...props}
    >
      <InnerContainer>
        {showIconLogo && (
          <S.LogoCircle shadow={false} size={32}>
      ...
      {isAiMessage && !knowledgeBase?.length && (
        <TippyTooltip
          ...
          interactive={true}
          content={
            <TippyTooltip.FooterButton buttonText="More" onClick={() => openURLInANewTab(NO_MATCH_LEARN_MORE)}>
      ...
      {knowledgeBase?.length && (
        <TippyTooltip
          ...
          content={
            <TippyTooltip.FooterButton
              buttonText="More"
              onClick={() => openURLInANewTab(CMS_KNOWLEDGE_BASE_LEARN_MORE)}
            >
