...
import type { AlexaStageType } from '@/constants/platforms';
import type { AlexaExportJob, AlexaPublishJob } from '@/models';

import handlersService from './handlers';
import projectService from './project';

const alexaClient = {
  export: createJobService<AlexaExportJob.AnyJob, AlexaStageType>(
    `${ALEXA_SERVICE_ENDPOINT}/${EXPORT_RESOURCE_ENDPOINT}`
  ),
  ...
  publish: createJobService<AlexaPublishJob.AnyJob, AlexaStageType>(
    `${ALEXA_SERVICE_ENDPOINT}/${PUBLISH_RESOURCE_ENDPOINT}`
  ),
