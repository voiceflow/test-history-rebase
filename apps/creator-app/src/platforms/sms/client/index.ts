...
import { SMSPublishStageType, TwilioPrototypeStageType } from '@/constants/platforms';
import { SMSPublishJob, TwilioPrototypeJob } from '@/models';
...
const smsServiceClient = {
  ...
  publish: createJobService<SMSPublishJob.AnyJob, SMSPublishStageType>(`${GENERAL_SERVICE_ENDPOINT}/twilio/sms/publish`),
  ...
  setNumbers: (projectID: string, numbers: string[]) => generalService.post<void>(`twilio/sms/${projectID}/numbers`, { numbers }),
};
