...
export interface LoguxDispatch<A extends Action> {
  ...
  /**
   * resolve when local redux store is updated, but also send a sync action to the server
   * @param action
   * @param meta
   * @returns Promise when action has updated the redux store
   */
  partialSync<T extends A>(action: T, meta?: Partial<ClientMeta>): Promise<ClientMeta>;
   ...
   */
  crossTab<T extends A>(action: T, meta?: Partial<ClientMeta>): Promise<ClientMeta>;
...
export interface LoguxReduxOptions {
  /**
