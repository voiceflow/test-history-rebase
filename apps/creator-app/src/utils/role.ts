...
import { ADMIN_ROLES, ALL_VIRTUAL_ROLES, EDITOR_USER_ROLES, VIEWER_USER_ROLES, VIRTUAL_ROLE_STRENGTH, VirtualRole } from '@/constants/roles';

export const isVirtualRole = (role: Nullish<UserRole | VirtualRole>): role is VirtualRole => ALL_VIRTUAL_ROLES.includes(role as any);

export const isEditorUserRole = (role: Nullish<UserRole | VirtualRole>): role is Realtime.ArrayItem<typeof EDITOR_USER_ROLES> =>
  EDITOR_USER_ROLES.includes(role as any);

export const isAdminUserRole = (role: Nullish<UserRole | VirtualRole>): role is Realtime.ArrayItem<typeof ADMIN_ROLES> =>
  ADMIN_ROLES.includes(role as any);

export const isViewerUserRole = (role: UserRole): role is Realtime.ArrayItem<typeof VIEWER_USER_ROLES> => VIEWER_USER_ROLES.includes(role);
