...
export class AssistantPublicHTTPController {
  ...
  @ZodApiResponse({ status: HttpStatus.OK, schema: AssistantExportCMSResponse })
  ...
  @Get('load-creator/:environmentID')
  @Authorize.Permissions<Request<{ environmentID: string }>>([Permission.PROJECT_READ], async (request) => ({
    id: await appRef.current.get(AssistantService).resolveEnvironmentIDAlias(request),
    kind: 'version',
  }))
  ...
  @ZodApiResponse({ status: HttpStatus.OK, schema: AssistantLoadCreatorResponse })
  @UseInterceptors(ResolveEnvironmentIDAliasInterceptor)
  loadCreator(
    @UserID() userID: number,
    @Param('environmentID') environmentID: string
  ): Promise<AssistantLoadCreatorResponse> {
    return this.service.loadCreator({ userID, environmentID });
  }
