...
export const broadcastContext = <T extends object = Record<string, never>>(
  assistant: PKOrEntity<AssistantEntity> | Ref<AssistantEntity>,
  extra?: T
): DesignerActionContext & T =>
  ({
    ...extra,
    assistantID: toEntityID(assistant),
    broadcastOnly: true,
  } as DesignerActionContext & T);
