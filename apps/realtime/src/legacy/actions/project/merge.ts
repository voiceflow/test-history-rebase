...
class MergeProjects extends AbstractProjectResourceControl<Realtime.project.MergeProjectsPayload, ContextData> {
  ...
  protected process = this.reply(Realtime.project.merge, async (ctx, { payload }) => {
    ...
    if (this.services.feature.isEnabled(Realtime.FeatureFlag.MIGRATION_V2)) {
    ...
    await Promise.all([
      this.server.processAs(
        creatorID,
        clientID,
        negotiateAction({ versionID: sourceProject.devVersion, proposedSchemaVersion: Realtime.LATEST_SCHEMA_VERSION })
      ),
      this.server.processAs(
        creatorID,
        clientID,
        negotiateAction({ versionID: targetProject.devVersion, proposedSchemaVersion: Realtime.LATEST_SCHEMA_VERSION })
      ),
    ]);
