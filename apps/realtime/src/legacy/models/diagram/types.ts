...
export interface FromDB {
  ...
  <DBDiagram extends Partial<DBDiagramModel>>(diagram: DBDiagram): Pick<
    BaseModels.Diagram.Model,
    Extract<keyof BaseModels.Diagram.Model, keyof DBDiagram>
  >;
}
...
export interface ToDB {
  ...
  <Diagram extends Partial<BaseModels.Diagram.Model>>(diagram: Diagram): Pick<DBDiagramModel, Extract<keyof DBDiagramModel, keyof Diagram>>;
}

export interface MapFromDB {
  (diagrams: DBDiagramModel[]): BaseModels.Diagram.Model[];

  <DBDiagram extends Partial<DBDiagramModel>>(diagrams: DBDiagram[]): Pick<
    BaseModels.Diagram.Model,
    Extract<keyof BaseModels.Diagram.Model, keyof DBDiagram>
  >[];
}
