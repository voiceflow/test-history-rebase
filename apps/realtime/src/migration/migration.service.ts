...
export class MigrationService {
  ...
  private getBackupName(schemeVersion: Realtime.SchemaVersion) {
    return `Before v${schemeVersion} (autogenerated)`;
  }
  ...
  public async *migrateSchema({
    ...
    const backupName = this.getBackupName(targetSchemaVersion);

    const backup = await this.backup.findOneByName(projectID, backupName);

    // this is a safety measure to ensure that user can rollback if something goes wrong
    // creates a backup if it does not exist - backup can exist if the migration failed and user is retrying
    if (!backup) {
      await this.backup.createOneForUser(creatorID, versionID, backupName);
    }
