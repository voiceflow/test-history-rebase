...
const migrateToV3_1: Transform = ({ version, diagrams }) => {
  ...
  if (!!version.topics?.length && version.topics.some((item) => String(item.sourceID) === String(version.rootDiagramID))) return;
  ...
  diagrams.forEach((diagram) => {
    const isRoot = String(diagram._id) === String(version.rootDiagramID);

    diagram.type = isRoot ? BaseModels.Diagram.DiagramType.TOPIC : diagram.type ?? BaseModels.Diagram.DiagramType.COMPONENT;
  });

  version.topics = [rootDiagramItem, ...(version.topics ?? [])];
  version.components = version.components?.filter((item) => String(item.sourceID) !== String(version.rootDiagramID)) ?? [];
};
