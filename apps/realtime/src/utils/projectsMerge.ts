...
class ProjectsMerge {
  ...
  private newNotes: Record<string, BaseModels.BaseNote> = {};
  ...
  private newComponents: BaseModels.Version.FolderItem[] = [];
  ...
  private mergeSlots() {
    const {
      platformData: { slots: sourceSlots },
    } = this.sourceVersion;
    const {
      platformData: { slots: targetSlots },
    } = this.targetVersion;
  ...
  private mergeIntents() {
    const {
      platformData: { intents: sourceIntents },
    } = this.sourceVersion;
    const {
      platformData: { intents: targetIntents },
    } = this.targetVersion;
