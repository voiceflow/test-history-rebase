  ...
  new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (existing_google_versions && Object.keys(existing_google_versions).length > 0) {
        ...
        highest_existing_version = +_.find(
          VALID_VERSION_REGEXES.map((regex) => highest_existing_version[highest_existing_version.length - 1].match(regex))
        )[1];
      } else {
      ...
      if (existing_google_versions)
        await pool.query('UPDATE project_members SET google_versions = $3 WHERE project_id = $1 AND creator_id = $2', [
          project_id,
          creator_id,
          existing_google_versions,
        ]);
