...
*/
const renderDiagram = (user, diagram_id, skill_id, options = {}, depth = 0, platform = 'alexa') =>
  new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (data.Item) {
        ...
        const diagram = JSON.parse(data.Item.data);
        let combine_nodes = [];
        ...
        for (let i = 0; i < diagram.nodes.length; i++) {
          ...
          if (node.extras.type === 'story') {
            ...
            const nextId = getLink(nextLink) || node.extras.nextID;
            const extras = node.extras[platform];

            if (extras) {
              // Default stop and helps don't have platform-specific extras
              if (extras.intent) {
                let { intent } = extras;
            ...
            const list = node.ports.filter((a) => !a.in && a.links.length > 0).map((port) => getLink(port.links[0]));
            story.lines[node.id] = {
            ...
            story.lines[node.id] = {
              ...
              nextIds: node.ports
                .filter((a) => !a.in && a.label !== 'else')
                .sort((a, b) => a.label - b.label)
                .map((port) => {
                  const link = getLink(port.links[0]);
                  return link || null;
                }),
            };
            ...
            ((node.extras.type === 'intent' && (node.extras.alexa && node.extras.alexa.choices)) || node.extras.choices)
          ) {
            const interactions = [];
            ...
            const NEXT = node.ports.find((a) => a.label === 'next');
            const PREVIOUS = node.ports.find((a) => a.label === 'previous');
            const PAUSE = node.extras.custom_pause && node.ports.find((a) => a.label === 'stop/pause' || a.label === 'pause');
            const gNEXT = node.ports.find((a) => a.label.trim() === '' && !a.in);
            ...
            node.extras.type === 'multiline' ||
            node.extras.type === 'line' ||
            node.extras.type === 'audio' ||
            ...
            if (node.extras.expressions) {
              story.lines[node.id] = {
                expressions: node.extras.expressions.map((expression) => {
                  const rendered = _expressionfy(expression);
            ...
            const id = hashids.decode(node.extras.template_id);
            if (id && id[0] && node.extras.to) {
              let mapping;
              if (Array.isArray(node.extras.mapping)) {
                mapping = node.extras.mapping.filter((m) => m.val && m.key);
              } else {
                mapping = [];
              }
            ...
            const id = hashids.decode(node.extras.display_id);
            story.lines[node.id] = {
              display_id: id[0],
            ...
            if (node.extras.a_l) {
              ...
              if (node.extras.custom && Array.isArray(node.extras.permissions)) {
                const permissions_array = node.extras.permissions.filter((p) => !!p.trim());
