...
const canvasUtils = {
  ...
  unStickCanvas: () => {
    cy.get(`#${Identifier.CANVAS}`).click();
  },
  clickCanvas: () => {
    cy.get(`#${Identifier.CANVAS}`).click();
  },
  ...
  clickHomeBlockPort: () => canvasUtils.getHomeBlock().find(`.${ClassName.CANVAS_PORT}`).click({ force: true }),
  getLastBlock: () => cy.get(`.${ClassName.CANVAS_BLOCK}`).last(),
  clickLastBlock: () => canvasUtils.getLastBlock().click({ force: true }),
  ...
  clickPreviousNthBlockPort: (n = 2, portNumber = 0) =>
    canvasUtils.getPreviousNthBlock(n).find(`.${ClassName.CANVAS_PORT}`).eq(portNumber).click({ force: true }),
  getLastPlacedPort: () => canvasUtils.getLastStep().find(`.${ClassName.CANVAS_PORT}`).eq(-1),
  clickLastPlacedPort: (portNumber = 0) => canvasUtils.getLastPlacedPort().eq(portNumber).click({ force: true }),
  connectLastBlockWithRecent: (portNumber = 0) => {
    canvasUtils.clickPreviousNthBlockPort(2, portNumber);
    canvasUtils.clickLastBlock();
  },
  focusLastStep: () => {
    canvasUtils.getLastStep().click('top', { force: true });
  },
  editor: {
