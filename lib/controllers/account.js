...
module.exports = function AccountController(services) {
   ...
   * @returns {string}
   */
  self.getAmazonCode = (req) => accountManager.createAmazonSession(req.user.id, req.params.code);
  ...
  self.getVendor = async (req) => {
    const vendors = await accountManager.getVendors(req.user.id);

    if (vendors.length === 0) {
      throw new VError('no vendor associated with account', VError.HTTP_STATUS.NOT_FOUND);
    }
    if (req.query.all) {
      return vendors;
    }
  ...
  self.deleteSession = (req) => req.user && accountManager.deleteSession(req.user.id, req.cookies);

  self.putSession = async (req) => {
    const account = await accountManager.checkLogin(req.body.user);
  ...
  self.googleLogin = async (req) => {
    const account = await accountManager.googleLogin(req.body.user);
    const { userHash, token, user } = await accountManager.createSession(account);
    return {
      token: userHash + token,
      user,
  ...
  self.facebookLogin = async (req) => {
    const account = await accountManager.facebookLogin(req.body.user);
    const { userHash, token, user } = await accountManager.createSession(account);
    return {
      token: userHash + token,
      user,
  ...
  self.putUser = async (req) => {
    const account = await accountManager.createUser(req.body.user);
    const { userHash, token, user } = await accountManager.createSession(account);
    return {
      token: userHash + token,
      user,
