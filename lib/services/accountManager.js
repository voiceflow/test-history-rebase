...
function AccountManager(services, config = {}) {
  ...
  const { CONFIG_CLIENT_ID, CONFIG_CLIENT_SECRET, NODE_ENV, GOOGLE_ID, FB_ID } = config;
  ...
  self.facebookAuth = async (code) => {
    const result = await axios.get('https://graph.facebook.com/v3.3/debug_token', {
      params: {
        input_token: code,
        access_token: FB_ID,
      },
   ...
   * @param {number} creatorId
   */
  self.getVendors = async (creatorId) => {
