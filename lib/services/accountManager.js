...
function AccountManager(services, config) {
  ...
  check(services, 'mailManager', 'object');
   ...
   * @returns {object} user row
   */
  self.facebookLogin = async ({ name, email, fbId, uri, code }) => {
  ...
  self.resetPasswordEmail = async (email) => {
    ...
    if (account) {
      const token = await self.generateLimitToken(account.creatorId, RESET_PASSWORD_PREFIX, 3);

      if (token) {
        await mailManager.sendResetEmail(account.name, `${token}${account.creator_id}`, email);
      }
    }
  };
