...
class AdminManager {
  ...
  async getCreatorData(userId) {
    ...
    const returnedUserDetails = await this.services.pool.query(
      `
    ...
    if (returnedUserDetails) {
      [returnData.userDetails] = returnedUserDetails.rows;
    }

    const userProjects = await this.services.pool.query(
      `
    ...
    // returnData.projects = userProjects.rows;

    // This will be a representation of the teams object in the teams
    const boards = {};
    userProjects.rows.forEach((p) => {
      if (!boards[p.team_id]) {
        boards[p.team_id] = {
          team_id: p.team_id,
          name: p.board_name,
