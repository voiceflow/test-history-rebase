...
class LinkManager {
  ...
  async getTemplate(skillId) {
    ...
    const result = await this.services.pool.query('SELECT account_linking, skill_id FROM skills WHERE skill_id = $1 LIMIT 1', [skillId]);
  ...
  async setTemplate(skillId, payload) {
    ...
    if (accountLinking.clientSecret) {
      accountLinking.clientSecret = this.services.jwt.sign(accountLinking.clientSecret);
    }

    const result = await this.services.pool.query('UPDATE skills SET account_linking = $1 WHERE skill_id = $2 RETURNING *', [
      accountLinking,
      skillId,
    ]);
