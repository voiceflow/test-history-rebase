...
export class Migration20240604164940 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      'create index "response_message_environment_id_index" on "designer"."response_message" ("environment_id");'
    );
    this.addSql(
      'alter table "designer"."response_message" add constraint "response_message_id_environment_id_unique" unique ("id", "environment_id");'
    );

    this.addSql(
      'alter table "designer"."response_message" add constraint "response_message_updated_by_id_foreign" foreign key ("updated_by_id") references "identity"."user" ("id") on update cascade on delete set null;'
    );
    this.addSql(
      'alter table "designer"."response_message" add constraint "response_message_assistant_id_foreign" foreign key ("assistant_id") references "designer"."assistant" ("id") on update cascade on delete cascade;'
    );
    this.addSql(
      'alter table "designer"."response_message" add constraint "response_message_discriminator_id_environment_id_foreign" foreign key ("discriminator_id", "environment_id") references "designer"."response_discriminator" ("id", "environment_id") on update cascade on delete cascade;'
    );
    this.addSql(
      'alter table "designer"."response_message" add constraint "response_message_condition_id_environment_id_foreign" foreign key ("condition_id", "environment_id") references "designer"."condition" ("id", "environment_id") on update cascade on delete set default;'
    );
