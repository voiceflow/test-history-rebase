...
import type { SystemVariable } from '@voiceflow/dtos';
...
export type ExtendableRequired<Config extends AnyRecord> = {
  [Key in keyof Config]: Config[Key] extends Icon
    ? // if value extends icon but has a strict icon type, override it to more generic icon type
      ExtendableRequired<Omit<Config[Key], 'name'> & { name: SvgIconTypes.Icon }>
    : Config[Key] extends { globalVariables: SystemVariable[] }
      ? // if value extends globalVariables but has a strict globalVariables type, override it to more generic globalVariables type
        ExtendableRequired<Omit<Config[Key], 'globalVariables'> & { globalVariables: SystemVariable[] }>
      : Config[Key] extends { default: string }
        ? // if value extends default voice but has a strict default type, override it to more generic default type
          ExtendableRequired<Omit<Config[Key], 'default'> & { default: string }>
        : Config[Key] extends AnyFunc
          ? // if value is a function
            AnyFunc
          : Config[Key] extends AnyRecord
            ? // if value is an object, recursively extend it
...
export type ExpectValidArgs<Func1 extends AnyFunc, Func2 extends AnyFunc> =
  Parameters<Func2> extends Parameters<Func1> ? true : false;
export type ExpectValidReturnType<Func1 extends AnyFunc, Func2 extends AnyFunc> =
  ReturnType<Func2> extends ReturnType<Func1> ? true : false;
...
export type Merge<Config extends AnyRecord, ExtendConfig extends AnyRecord> = ExtendConfig &
