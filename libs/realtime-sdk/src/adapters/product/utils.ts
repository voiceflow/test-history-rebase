...
export const getDistributionCountries = (
...
export const formatMarketPlaces = (
  ...
  const generalReleaseDate =
    Object.values(marketPlaces).find((place) => !!place?.releaseDate)?.releaseDate || dayjs().format('YYYY-MM-DD');

  return Utils.object
    .getKeys(marketPlaces)
    .reduce<Partial<Record<AlexaProject.MarketPlace, AlexaProject.PublishingPrice>>>((acc, key) => {
...
export const parseMarketPlaces = (
  ...
  Utils.object
    .getKeys(allPlaces)
    .reduce<Partial<Record<AlexaProject.MarketPlace, ProductMarketPlace>>>((acc, encodedKey) => {
