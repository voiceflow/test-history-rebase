import type { Server, ServerMeta } from '@logux/server';
import type { ControlOptions } from '@socket-utils/control';
import { AbstractControl } from '@socket-utils/control';
...
export class SyncService extends AbstractControl<ControlOptions<SyncServiceConfig>> {
  ...
  public start(server: Server): void {
    ...
    const pubsubUnsubscribe = this.clients.pubsub.subscribe<[AnyAction, ServerMeta]>(
      this.config.LOGUX_ACTION_CHANNEL,
      ([action, meta]) => {
