...
export const findCreator =
  (user: string): Thunk<Nullable<Creator>> =>
  async (dispatch) => {
...
export const editTrial =
  (workspaceID: number, date: number): Thunk =>
  async () => {
    if (!workspaceID) {
      return;
    }
...
export const refundCharge =
  ({ workspaceID, chargeID, chargeAmount }: { workspaceID: number; chargeID: string; chargeAmount: number }): Thunk =>
  async () => {
...
export const cancelSubscription =
  (workspaceID: number, subscriptionID: string): Thunk =>
  async () => {
...
export const updateWorkspaceData =
  (workspaceID: number, data: Partial<Workspace>): SyncThunk =>
  async (dispatch) => {
    if (!workspaceID) {
...
export const updateWorkspaceMemberRole =
  ({ workspaceID, creatorID, role }: { workspaceID: number; creatorID: number; role: string }): SyncThunk =>
  async (dispatch) => {
    if (!workspaceID) {
      toast.error('Workspace not found');
      return;
