...
export const addByKeyPortReverter = createReverter(
  ...
  ({ workspaceID, projectID, versionID, diagramID, nodeID, portID, key }) =>
    Realtime.port.removeByKey({ workspaceID, projectID, versionID, diagramID, nodeID, portID, key }),

  [
    ...DIAGRAM_INVALIDATORS,
    ...createNodeRemovalInvalidators<Realtime.port.AddByKeyPayload>((origin, nodeID) => origin.nodeID === nodeID),
    createDiagramInvalidator(Realtime.port.addByKey, (origin, subject) => origin.nodeID === subject.nodeID && origin.key === subject.key),
  ]
);
