...
export const loadThread =
  (projectID: string, threadID: string): Thunk<Thread> =>
  async (dispatch) => {
...
export const createThread =
  ({ nodeID, position, data }: { data: Pick<Comment, 'text' | 'mentions'>; nodeID: string | null; position: Pair<number> }): Thunk<Thread> =>
...
export const updateThreadData =
  (threadID: string, data: Partial<Pick<Thread, 'resolved' | 'position' | 'nodeID' | 'deleted'>>): Thunk =>
  async (dispatch, getState) => {
    const state = getState();
    const projectID = Session.activeProjectIDSelector(state);
...
export const createComment =
  (threadID: string, data: Partial<Pick<Comment, 'text' | 'mentions'>>): Thunk =>
  async (dispatch, getState) => {
    const state = getState();
    const projectID = Session.activeProjectIDSelector(state);
    const creatorID = Account.userIDSelector(state);
...
export const updateComment =
  (threadID: string, commentID: string, data: Pick<Comment, 'text' | 'mentions'>): Thunk =>
  async (dispatch, getState) => {
    const state = getState();
    const projectID = Session.activeProjectIDSelector(state);
    const thread = ThreadSelectors.threadByIDSelector(state)(threadID);
...
export const handleThreadUpdate =
...
export const handleNewReply =
  (payload: { projectID: string; created: DBComment }): SyncThunk =>
  (dispatch, getState) => {
    const state = getState();
...
export const handleCommentUpdate =
  (payload: { projectID: string; updatedComment: DBComment }): SyncThunk =>
  (dispatch, getState) => {
    const state = getState();
...
export const handleCommentDelete =
  (payload: { projectID: string; commentID: string; threadID: string }): SyncThunk =>
  (dispatch, getState) => {
    const state = getState();
