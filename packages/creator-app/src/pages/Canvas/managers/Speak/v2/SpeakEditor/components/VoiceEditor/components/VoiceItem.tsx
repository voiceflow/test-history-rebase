...
export interface VoiceItemProps
  extends DragPreviewComponentProps,
    ItemComponentProps<Realtime.SSMLData>,
    MappedItemComponentHandlers<Realtime.SSMLData> {
  editor: NodeEditorV2Props<Realtime.NodeData.Speak, Realtime.NodeData.SpeakBuiltInPorts>;
  latestCreatedKey: string | undefined;
}

const VoiceItem: React.ForwardRefRenderFunction<HTMLElement, VoiceItemProps> = (
  ...
  return (
    <EditorV2.PersistCollapse namespace={['speak-voice-editor-item', item.id]} defaultCollapsed={!autofocus}>
      {({ collapsed, onToggle }) => (
        <>
              ...
              <SectionV2.CollapseSection
                ...
                header={
                  <SectionV2.Header ref={connectedDragRef} sticked={sticked && !collapsed && !isDraggingPreview && !isDragging}>
                    <SectionV2.Title bold={!collapsed}>Speak variant {index + 1}</SectionV2.Title>
...
export default React.forwardRef<HTMLElement, VoiceItemProps>(VoiceItem);
