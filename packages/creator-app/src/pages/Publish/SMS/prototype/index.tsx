...
const SMSTesting: React.OldFC = () => {
  ...
  useAsyncEffect(async () => {
    const projectSecret = await client.apiV3.projectSecret.findByProjectID(projectID, ProjectSecretTag.SMS_PHONE_NUMBER);
  ...
  return (
    <SettingsContainer>
      ...
      <Section title="Testing Number">
        ...
        <Box.Flex gap={24}>
          <PhoneInput defaultCountry="US" error={error} placeholder="Enter number" value={value} onChange={setValue} onBlur={updateValue} />
          <Box flexBasis="48%" flexShrink={0} fontSize={13}>
            The phone number you'll use to test your assistant.
          </Box>
        </Box.Flex>
      </Section>
    </SettingsContainer>
