  ...
  describe('selectors', () => {
    ...
    describe('allSlotsSelector()', () => {
      it('select all slots from the legacy store', () => {
        ...
        const result = Slot.allSlotsSelector(createState(MOCK_STATE, { [SlotV1.STATE_KEY]: normalize(slots) }));
    ...
    describe('allSlotIDsSelector()', () => {
      it('select all slot IDs from the legacy store', () => {
        ...
        const result = Slot.allSlotIDsSelector(createState(MOCK_STATE, { [SlotV1.STATE_KEY]: normalize(slots) }));
    ...
    describe('slotMapSelector()', () => {
      it('select slot map from the legacy store', () => {
        const slotState = normalize(Utils.generate.array(3, () => ({ id: Utils.generate.id() })));
    ...
    describe('slotsByIDsSelector()', () => {
      it('select slots from the legacy store', () => {
        ...
        const slotState = normalize([otherSlot, slot]);
