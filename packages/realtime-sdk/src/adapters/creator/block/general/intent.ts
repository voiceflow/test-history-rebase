...
const intentAdapter = createBlockAdapter<Node.Intent.StepData, NodeData.Intent>(
  ({ intent, mappings, availability }) => ({
    ...
    [Constants.PlatformType.GENERAL]: { intent, mappings: mappings ?? [], availability: availability ?? Node.Intent.IntentAvailability.GLOBAL },
  }),
  ({ [Constants.PlatformType.GENERAL]: { intent, mappings, availability } }) => ({ intent, mappings, availability })
);
