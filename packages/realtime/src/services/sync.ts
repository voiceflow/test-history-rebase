...
class SyncService extends AbstractControl {
  ...
  public start(server: Server): void {
    // listen to messages on all other "realtime:actions" channels and re-broadcast them to connected clients
    const pubsubUnsubscribe = this.clients.pubsub.subscribe<[AnyAction, ServerMeta]>(CHANNEL, ([action, meta]) => server.sendAction(action, meta));

    // listen to actions from this server's event log and publish them to our "realtime:actions" channel
