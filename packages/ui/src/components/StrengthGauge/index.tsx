...
const StrengthGauge: React.OldFC<T.Props> = ({ level = Level.NOT_SET, width = 100, tooltipLabelMap }) => (
  <S.Container content={tooltipLabelMap?.[level] || TOOLTIP_LABEL_MAP[level]} offset={[0, 8]}>
    <ProgressBar width={width} color={StrengthColor[level]} height={2} progress={LINE_MULTIPLIER_MAP[level]} />
  </S.Container>
);
