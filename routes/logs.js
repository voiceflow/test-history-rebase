...
exports.getLogsUser = async (req, res) => {
  try {
    ...
    `,
      [req.user.id, version_id]
    )).rows[0];

    if (!base) throw { status: 404 };

    const logs = (await logging_pool.query('SELECT timestamp, user_id, request FROM logs WHERE amzn_id = $1 ORDER BY timestamp DESC', [base.amzn_id]))
      .rows;
