...
const getTeamId = (project_id) => new Promise(async (resolve) => {
  try {
...
const updateModuleInES = (module_data) => new Promise(async (resolve, reject) => {
...
const saveCertification = async (req, res) => {
  ...
  const createNewModule = async () => {
    ...
    try {
      const new_module_data = (await pool.query('INSERT INTO projects (name, creator_id) VALUES ($1, $2) RETURNING *', [req.body.title, ADMIN_MARKETPLACE_ACC])).rows[0];
      await pool.query(
        `INSERT INTO modules 
...
const requestCertification = async (req, res) => {
  ...
  try {
    ...
    copySkill(req, res, {
      user_copy: true, request_cert: true, project_id: module_project_id, creator_id: ADMIN_MARKETPLACE_ACC,
    }, () => {
      analytics.track({
        userId: req.user.id,
        event: 'Flow Created',
