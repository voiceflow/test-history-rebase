...
exports.renderDisplay = async (req, res) => {
  try {
    ...
    const { document } = (await pool.query('SELECT document FROM displays WHERE id = $1 LIMIT 1', [id])).rows[0];

    const result = await axios.post('http://ip-10-0-1-195.ec2.internal:8080/render', {
      document,
