...
exports.getProjectFromSkill = async (req, res, next) => {
  try {
    ...
    const { project_id } = (await pool.query('SELECT project_id FROM skills WHERE skill_id = $1 LIMIT 1', [skill_id])).rows[0];
...
exports.getLiveVersion = async (req, res) => {
  ...
  try {
    ...
    if (live_version_data.rows.length > 0) {
      live_version = hashids.encode(live_version_data.rows[0].skill_id);
      live_skill = live_version_data.rows[0];
    }
    res.send({ live_version, live_skill });
  } catch (err) {
    console.trace(err);
    res.sendStatus(500);
  }
