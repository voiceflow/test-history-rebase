...
const { pool, hashids, intercom, jwt, logAxiosError, writeToLogs, analytics } = require('./../services');
const { AccessToken, AmazonAccessToken, _getGoogleAccessToken } = require('./authentication');
const { createManifest } = require('./../config/manifest');
const { createInteractionModel } = require('./../config/interaction_model');
const { generateDialogflowPackage } = require('./../config/gactions_package');
const { deleteVersionPromise, copySkill, deleteSkillDiagramsPromise } = require('./skill_util');
const { pg_num } = require('./../util');
...
const incrementSkillsCreatedIntercom = (id) => {
  ...
  intercom.users.find(
    ...
    async (res) => {
      ...
      if (!sc) {
        const data = await pool.query('SELECT * FROM skills WHERE creator_id = $1', [id]);
...
const incrementTimesPublishedIntercom = (id) => {
  ...
  intercom.users.find(
    ...
    async (res) => {
      ...
      const n = res.body.custom_attributes.times_published ? res.body.custom_attributes.times_published : 0;
      intercom.users.create({
        user_id: id,
        custom_attributes: {
          times_published: n + 1,
...
const incrementTimesPublishedSuccessfulIntercom = (id) => {
  ...
  intercom.users.find(
    ...
    async (res) => {
      ...
      const n = res.body.custom_attributes.times_published_successful ? res.body.custom_attributes.times_published_successful : 0;
      intercom.users.create({
        user_id: id,
        custom_attributes: {
...
const checkSkillInReview = (amzn_id, creator_id, skill_id) => {
  ...
  return new Promise(async (resolve) => {
    AmazonAccessToken(creator_id).then(async (token) => {
      ...
      try {
        ...
        if (res.data.items.find((item) => item.status === 'IN_PROGRESS') !== undefined) {
...
const makeVendorEntryForExistingSkill = async (creator_id, project_id, amzn_id) => {
  ...
  const { vendors } = (await axios.request({
  ...
  for (vendor of vendors) {
    ...
    while (true) {
      const { skills, nextToken, isTruncated } = (await axios.request({
        url: `https://api.amazonalexa.com/v1/skills?vendorId=${id}&maxResults=50&nextToken=${nToken || ''}`,
        method: 'GET',
        headers: {
          Authorization: token,
        },
      ...
      if (_.find(skills, { skillId: amzn_id })) {
        ownerVendor = id;
        break;
      }
    ...
    if (ownerVendor) break;
  }

  if (ownerVendor) {
    await pool.query('INSERT INTO vendors (project_id, creator_id, vendor_id, amzn_id) VALUES ($1, $2, $3, $4)', [
      project_id,
      creator_id,
      ownerVendor,
      amzn_id,
...
exports.getSkill = async (req, res) => {
  ...
  try {
    ...
    if (!_.isNil(skill_data.deprecated_amzn_id) && _.isNil(skill_data.amzn_id)) {
      ...
      const rows = await pool.query('SELECT * FROM vendors WHERE creator_id = $1 AND project_id = $2 AND amzn_id = $3', [
        req.user.id,
        project_id,
...
exports.getDiagrams = (req, res) => {
  ...
  const sql = `SELECT d.id, d.name, d.sub_diagrams, d.module_id FROM diagrams d
...
exports.getProducts = (req, res) => {
  ...
  const sql = 'SELECT id, name, data FROM products WHERE skill_id = $1';
...
exports.getProduct = (req, res) => {
  ...
  const sql = 'SELECT id, name, data FROM products WHERE skill_id = $1 AND id = $2';
...
exports.setProduct = async (req, res) => {
  ...
  try {
    if (req.query.new) {
      pool.query(
        'INSERT INTO products (name, skill_id, data) VALUES ($1, $2, $3) RETURNING id',
        [product.name, product.skill, product.data],
        (err, results) => {
          res.status(200).send({
...
exports.deleteProduct = async (req, res) => {
  ...
  try {
    ...
    products.forEach(async (product) => {
      ...
      await AmazonAccessToken(product.creator_id).then((token) => {
        if (!token) return;
        axios.request({
...
exports.patchSkill = async (req, res) => {
  ...
  try {
    if (req.query.fulfillment) {
      ...
      const old_inv_data = (await pool.query('SELECT inv_name, invocations FROM skills WHERE skill_id = $1', [id])).rows[0];
      const old_inv_name = old_inv_data.inv_name;
      const old_invocations = JSON.stringify(old_inv_data.invocations);
      const new_invocations = old_invocations.replace(new RegExp(old_inv_name, 'g'), b.inv_name);
...
const checkVersions = (project_id, platform, options = {}) =>
  new Promise(async (resolve) => {
    ...
    `,
      ...
      while (i < project_versions.length && num_versions_to_delete > 0) {
        ...
        if (!live_ids.has(v.skill_id) && creators.has(v.creator_id)) {
...
exports.buildSkill = async (req, res) => {
  const project_id = hashids.decode(req.params.project_id)[0];
  const id = hashids.decode(req.params.version_id)[0];
  const original_id = req.params.version_id;
  ...
  AccessToken(req.user.id, (token) => {
    ...
    pool.query(
      `
      SELECT s.*, pm.creator_id AS status, pm.selected_vendor as vendor_id
      FROM skills s
        ...
        if (err || data.rowCount === 0) {
          ...
          if (_.isNil(vendor_id)) {
            ...
            if (Array.isArray(vendors) && vendors.length !== 0) {
              ...
              await pool.query('UPDATE project_members SET selected_vendor = $3 WHERE creator_id = $1 AND project_id = $2', [
                req.user.id,
                project_id,
                vendor_id,
              ]);
          ...
          const q = await pool.query('SELECT amzn_id FROM vendors WHERE vendor_id = $1 AND creator_id = $2 AND project_id = $3', [
            vendor_id,
            req.user.id,
            project_id,
          ]);
          ...
          try {
            if (amzn_id) {
              try {
                ...
                if (request.data.manifest && request.data.manifest.lastUpdateRequest && request.data.manifest.lastUpdateRequest.status === 'FAILED') {
                  amzn_id = null;
                }
              } catch (err) {
            ...
            try {
              if (!amzn_id) {
                ...
                const update = await pool.query('UPDATE vendors SET amzn_id = $4 WHERE vendor_id = $1 AND creator_id = $2 AND project_id = $3', [
                  vendor_id,
                  req.user.id,
                  project_id,
                  amzn_id,
                ...
                if (update.rowCount === 0) {
                  await pool.query('INSERT INTO vendors (vendor_id, creator_id, project_id, amzn_id) VALUES ($1, $2, $3, $4)', [
                    vendor_id,
                    req.user.id,
                    project_id,
                    amzn_id,
              ...
              if (Array.isArray(products.rows) && products.rows.length !== 0) {
                for (const row of products.rows) {
                  ...
                  try {
                    ...
                    const product_response = await axios.request({
                    ...
                    if (AmazonProductId !== row.amzn_prod_id) {
                      if (row.status) {
                        await pool.query('UPDATE product_creators SET amzn_prod_id = $1 WHERE product_id = $2 AND creator_id = $3', [
                          AmazonProductId,
                          productId,
                          req.user.id,
                        ]);
                      } else {
                        await pool.query('INSERT INTO product_creators (product_id, creator_id, amzn_prod_id) VALUES ($1, $2, $3)', [
                          productId,
                          req.user.id,
                          AmazonProductId,
                        ]);
              ...
              if (depth === 3) {
                ...
                setTimeout(async () => {
                  ...
                  for (const locale of r.locales) {
                    ...
                    if (!(lang in models)) {
                      const { model, samples } = createInteractionModel(r, locale);
...
exports.restoreSkillVersion = async (req, res) => {
  ...
  let dev_version;
  let team_id;
  const restore_id = hashids.decode(req.params.restore_id)[0];
  try {
  ...
  copySkill(
    ...
    async (row) => {
      try {
        ...
        const new_skill_id = hashids.decode(row.skill_id)[0];
  ...
  pool.query(
    ...
    async (err, data) => {
      if (err) {
        ...
        const publish_info = data.rows[0].google_publish_info;
        ...
        if (data.rows[0].dialogflow_token) {
          try {
            const { dialogflow_token } = data.rows[0];
        ...
        const { created, diagram, privacy_policy, terms_and_cond } = data.rows[0];
