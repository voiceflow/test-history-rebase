  ...
  new Promise(async (resolve, reject) => {
    try {
      ...
      for (let i = 0; i < diagram_data_rows.length; i++) {
        // To0 f4st for 4mzn
        setTimeout(() => {
          diagram_delete_promises.push(exports.deleteDynamoDiagramPromise(diagram_data_rows[i].id));
        }, 20);
...
const copyProducts = (old_skill_id, new_skill_id) =>
  new Promise(async (resolve, reject) => {
    const select_query = `
...
const copyEmailTemplates = (old_skill_id, new_skill_id, new_creator_id) =>
  new Promise(async (resolve, reject) => {
    const select_query = `
...
const copyDisplays = (old_skill_id, new_skill_id, new_creator_id) =>
  new Promise(async (resolve, reject) => {
    const select_query = `
...
const uploadCopiedDiagram = (data, new_skill_id, new_creator_id, diagram_names, module_id) =>
  new Promise(async (resolve, reject) => {
    const params = {
      TableName: process.env.DIAGRAMS_DYNAMO_TABLE,
      Item: {
...
const renderSkill = async (skill, user) => {
  ...
  try {
    ...
    await pool.query('UPDATE skills set used_intents = $2, used_choices = $3, alexa_permissions = $4, alexa_interfaces = $5 WHERE skill_id = $1', [
      skill.skill_id,
      JSON.stringify([...used_intents]),
      JSON.stringify(used_choices),
        ...
        if (options.copying_default_template || options.user_copy) {
          try {
            if (options.request_cert && copy_skill) {
              await pool.query('UPDATE skills SET project_id = $1, cert_requested = NOW() WHERE skill_id = $2', [
                options.project_id,
                copy_skill.skill_id,
              ]);
            } else if (copy_skill) {
              ...
              const new_project_data = (await pool.query(
                ...
                VALUES ($1, $2, $3, $4) 
                RETURNING *`,
                [options.name, copy_skill.creator_id, copy_skill.skill_id, team_id]
              )).rows[0];
...
const copyIntentsAndSkills = (origin_skill_id, dest_skill_id) =>
  new Promise(async (resolve, reject) => {
    try {
      const origin_skill = (await pool.query('SELECT * FROM skills WHERE skill_id = $1', [origin_skill_id])).rows[0];
      const dest_skill = (await pool.query('SELECT * FROM skills WHERE skill_id = $1', [dest_skill_id])).rows[0];

      const new_intents = dest_skill.intents.concat(origin_skill.intents);
      const new_slots = dest_skill.slots.concat(origin_skill.slots);

      await pool.query('UPDATE skills SET intents = $1, slots = $2 WHERE skill_id = $3', [
        JSON.stringify(new_intents),
...
exports.copyDiagramsFromSkill = async (origin_skill_id, dest_skill_id, new_creator_id, new_flow_name, module_id, copy_intents_and_skills) =>
  new Promise(async (resolve, reject) => {
    const diagram_names = {};
    const diagram_mapping = {};
    let remapped_products;
    ...
    try {
      ...
      let dest_diagram_names = (await pool.query(
        'SELECT diagrams.name FROM diagrams INNER JOIN skills ON diagrams.skill_id = skills.skill_id WHERE skills.skill_id = $1',
        [dest_skill_id]
      )).rows;
