    ...
    return (
      <div>
        <div className="row internalIdSearchHeader">
          <div className="internalIdSearchField">
            ...
            <div>
              <Input
                ...
                placeholder="Enter a user id or email"
                onKeyDown={(e) => {
                  if (e.key === 'Enter') this.lookupUserById();
                }}
              />
            </div>
            <div className="internalIdSearchButton">
              <Button color="primary" className="w-30" isPrimarySmall onClick={this.lookupUserById}>
                Search
              </Button>
            </div>
          ...
          <div className="internalIdSearchUserResult">{this.props.creator.creator_id ? <UserCard /> : null}</div>
        </div>

        <div className="internalIdSearchResults">
          {this.props.boards.length > 0 ? <h4>Boards:</h4> : null}
          <div>{this.renderBoards()}</div>
        </div>
      </div>
    );
  }
...
const mapStateToProps = {
  ...
  boards: Admin.boardsSelector,
  errorMessage: Admin.errorSelector,
};
...
export default connect(mapStateToProps, mapDispatchToProps)(InternalLookup);
