...
import { Referral } from '@/admin/client';
import { Option } from '@/admin/constants';
import { Referral as ReferralType, StripeProduct } from '@/admin/models';
import { ReferralForm } from '@/admin/pages/Referral/types';
import { SyncThunk, Thunk } from '@/admin/store/types';
...
export const setReferral = (formData: ReferralForm, callback: () => void): Thunk => async (dispatch, getState) => {
...
export const updateReferral = (data: Pick<ReferralForm, 'status' | 'code'>): Thunk => async (dispatch) => {
  try {
...
export const getReferrals = (filter: Option): SyncThunk<ReferralType[]> => (_, getState) => {
  const state = getState();
