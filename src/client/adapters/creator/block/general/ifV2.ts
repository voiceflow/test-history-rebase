...
import { ExpressionData as NodeDataExpressionData, NodeData } from '@/models';

import { createBlockAdapter, defaultPortAdapter, PortsAdapter } from '../utils';

export const ifAdapterV2 = createBlockAdapter<IfData, NodeData.If>(
  ({ expressions }) => ({
    expressions: expressionAdapterV2.mapFromDB(expressions),
  }),
  ({ expressions }) => ({
    expressions: expressionAdapterV2.mapToDB(expressions as NodeDataExpressionData[]),
  })
);

export default ifAdapterV2;

export const ifPortsAdapter: PortsAdapter<NodeData.If> = {
  toDB: (ports, _, data) => {
    const dbPorts = defaultPortAdapter.toDB(ports, _, data);

    // assign path data to port data
    return dbPorts.map((port, index) => {
      const expressionTitle = (data.expressions[index] as NodeDataExpressionData)?.name;
