...
const commentClient = {
  ...
  update: (projectID: string, commentID: string, data: Comment) =>
    fetch.put(`${COMMENTING_PATH}/${projectID}/comment/${commentID}`, commentAdapter.toDB(data)),

  delete: (projectID: string, commentID: string) => fetch.delete(`${COMMENTING_PATH}/${projectID}/comment/${commentID}`),
};
