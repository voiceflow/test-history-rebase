...
const diagramClient = {
  getData: (diagramID: string, platform: PlatformType, isBlockRedesignEnabled: boolean): Promise<unknown> =>
  ...
  get: (diagramID: string): Promise<unknown> => fetch(`${DIAGRAM_PATH}/${diagramID}`).then(({ diagram }) => diagramAdapter.fromDB(diagram)),

  create: (diagram: unknown) => fetch.post(`${DIAGRAM_PATH}?new=1`, diagram),

  copy: (diagramID: string, name: string) => fetch(`${DIAGRAM_PATH}/copy/${diagramID}?name=${encodeURI(name)}`),
  ...
  update: (diagram: unknown) => fetch.post(`${DIAGRAM_PATH}`, diagram),

  findVariables: (diagramID: string): Promise<string[]> => fetch(`${DIAGRAM_PATH}/${diagramID}/${VARIABLES_PATH}`),
