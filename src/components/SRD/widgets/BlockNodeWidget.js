...
export class BlockNodeWidget extends BaseWidget {
  ...
  addBlocks = (e) => {
    ...
    this.props.nodeProps.setBlockMenu(
      <React.Fragment>
        <div
          style={{
            top: engine.getDiagramModel().getGridPosition(e.clientY - 115),
            left: engine.getDiagramModel().getGridPosition(newX - 115),
            cursor: 'pointer',
            position: 'absolute',
            zIndex: 10,
            width: '300px',
    ...
    if (port.parent) {
      return (
        <BlockPortLabel
          ...
          isMoving={this.props.node.isMoveInside}
        />
      );
    }
    return null;
  ...
  addTemp(e, isTop = false) {
    if (
      ...
      this.props.node.parentCombine.extras.type === 'god' &&
      e.buttons === 1 &&
      combineAppendValidation(this.props.node) &&
      appendValidator(this.props.diagramEngine.getSuperSelect())
    ) {
      if (
        (combineAppendValidation(_.last(this.props.node.parentCombine.combines)) ||
          combineAppendValidation(this.props.diagramEngine.getSuperSelect())) &&
        combineAppendValidation(this.props.diagramEngine.getSuperSelect())
      ) {
        ...
        combineAppendValidation(_.last(this.props.node.parentCombine.combines)) &&
        !combineAppendValidation(this.props.diagramEngine.getSuperSelect()) &&
        _.last(this.props.node.parentCombine.combines) !== 'temp'
      ) {
  ...
  removeTemp(e) {
    if (
      (this.props.node.parentCombine && _.includes(this.props.node.parentCombine.combines, 'temp')) ||
      (!_.isEmpty(this.props.node.combines) && _.includes(this.props.node.combines, 'temp'))
    ) {
      if (
        this.props.node.parentCombine &&
        combineAppendValidation(this.props.diagramEngine.getSuperSelect()) &&
        !this.props.diagramEngine.getSuperSelect().parentCombine
      ) {
  ...
  combineNode(e = null) {
    ...
    if (
      combineValidation(current, target_node, this.props.nodeProps.setCanvasError) &&
      (combineAppendValidation(current) || combineAppendValidation(target_node))
    ) {
  ...
  render() {
    ...
    const paddingStyle =
      this.props.node.extras.type === 'god' && combineAppendValidation(_.last(this.props.node.combines))
    ...
    return (
      <div
        ...
        onMouseUp={(e) => {
          if (
            this.props.diagramEngine.getSuperSelect() &&
            this.props.diagramEngine.getSuperSelect() !== this.props.node &&
            this.props.diagramEngine.getSuperSelect().isMoving &&
            !this.props.diagramEngine.getSuperSelect().isMoveInside
          ) {
            this.combineNode(e);
          }
        }}
      ...
      >
        {this.props.node.linter && this.props.node.linter.length > 0 && (
          <Tooltip
            ...
            html={
              <div className="linter-tooltip">
                {this.props.node.linter.map((s, i) => {
                  return (
                    <div className="linter-element d-flex justify-content-start" key={i}>
                      <div className="my-1 mx-1">
                        <span className="linter-number">{i + 1}</span>
                      </div>
                      <span className="linter-text">{s}</span>
                    </div>
                  );
                })}
              </div>
        ...
        {this.props.node.extras && this.props.node.extras.type === 'god' ? (
          <div
            className="w-100"
            style={{ height: 12 }}
            onMouseEnter={(e) => {
        ...
        <div
          className={this.bem('__title') + ' no-select'}
        ...
        >
          {this.props.node.extras.type === 'story' ? (
            <div className="home-block">
              {this.props.node.extras.type === 'story' && (
                <Tooltip className="float-left menu-tip-home" position="left" title="This is where your project begins" distance={18}>
                  <img src={'/home.svg'} height={13} width={13} alt="home" />
                </Tooltip>
              )}
              <div className="home-title">
                {this.props.nodeProps.diagram ? (this.props.nodeProps.diagram.name === 'ROOT' ? 'Home' : this.props.nodeProps.diagram.name) : 'Flow'}
              </div>
              <div className="faux-start-block">Start</div>
              {!!this.props.node.combines && !!this.props.node.combines.length && (
                <React.Fragment>
                  <hr />
                  <Tooltip
            ...
            >
              {this.props.node.extras.type === 'god' && (
                <Tooltip className="float-left menu-tip-home edit-name" position="left" title="Edit block name" distance={18}>
                  <img
                    src={'/edit-name.svg'}
                    height={13}
                    width={13}
              ...
              {this.props.node.extras.type === 'command' &&
                !!this.props.node.parentCombine &&
                this.props.node.parentCombine.extras.type === 'story' &&
                this.props.node.extras['alexa'] &&
                this.props.nodeProps.hasFlow(this.props.node.extras['alexa'].diagram_id) && (
                  <div className="command-right">
                    ...
                    >
                      <img src={'/flows.svg'} alt="flows" className="mr-2" width="10" />
                      Enter Flow
                    </Button>
                  </div>
                )}
        ...
        >
                ...
                if (!(node instanceof String) && node.id) {
                  ...
                  return React.createElement(
                    BlockNodeWidget,
                    {
                      ...
                      selected:
                        (this.props.diagramEngine.getSuperSelect() && this.props.diagramEngine.getSuperSelect().id === node.id) || node.selected,
                      ...
                      onClick: () => {
                        node.parentCombine = this.props.node;
                        this.props.diagramEngine.setSuperSelect(
                          new BlockNodeModel().deSerialize(node, this.props.diagramEngine, this.props.node, node.fade, node.linter, keepLink)
                        );
                      },
                    },
                    this.props.diagramEngine.generateWidgetForNode(
                      new BlockNodeModel().deSerialize(node, this.props.diagramEngine, this.props.node, node.fade, node.linter, keepLink)
                    )
                  );
                } else {
                  return (
                    <AnimateHeight key={idx} duration={1000} height={'auto'}>
                      <div className="rearrange-placeholder" style={{ height: '40px' }} key={idx} />
                    </AnimateHeight>
                  );
                }
              })}
          ...
          {this.props.node.extras.type === 'flow' && this.props.nodeProps.hasFlow(this.props.node.extras.diagram_id) && (
            <Button
              ...
              isBlack
              isSmall
              className="mt-1 mx-2 mb-3"
              onMouseDown={(e) => e.stopPropagation()}
              onMouseUp={() => this.props.nodeProps.enterFlow(this.props.node.extras.diagram_id)}
            >
              <img src={'/flows.svg'} alt="flows" style={{ marginTop: -1, marginRight: 5 }} width="10" />
              Enter Flow
            </Button>
          )}
        ...
        {(this.props.node.extras.type === 'story' ||
          ...
          <div
            id="add-command"
            onMouseUp={(e) => {
              this.props.node.extras.type === 'story' ? this.addCommand(e) : this.addBlocks(e);
            }}
          >
            <Tooltip position="bottom" title={this.props.node.extras.type === 'story' ? 'Add Command' : 'Add Step'} distance={18}>
              <Button isRound />
            </Tooltip>
          </div>
        )}
