  ...
  generatePort(port) {
    if (port.parent) {
      return <BlockPortLabel model={port} key={port.id} diagramEngine={this.props.diagramEngine}
                             isLast={this.props.isLast} isMoving={this.props.node.isMoveInside}/>;
    }
    return null;
  }
  ...
  render() {
    ...
    if (this.props.node.extras.type === 'comment') {
      return <div className={`srd-default-node ${this.props.node.extras.type}`}>
        <Textarea value={this.props.node.name} readOnly={this.props.locked} onChange={e => {
    ...
    return (
      ...
      >
        ...
        >
          {
            this.props.node.extras.type === 'story' ?
              <div className="home-block">
                ...
                <div
                  className="home-title">{this.props.nodeProps.diagram ? (this.props.nodeProps.diagram.name === 'ROOT' ? 'Home' : this.props.nodeProps.diagram.name) : 'Flow'}</div>
              ...
              <div className={this.bem("__name")} style={this.props.node.parentCombine ? {
                fontSize: '13px',
                textAlign: 'left',
                padding: '0 10px',
                fontWeight: '500'
        ...
        >
          <div
            className={`${this.bem("__in")} ${this.props.node.extras.type !== 'card' && this.props.node.extras.type}`}>
          ...
          }
          {
            ...
            <React.Fragment>
              <img className="rounded ModuleIcon" draggable={false} src={this.props.node.extras.module_icon}
                   alt={this.props.node.extras.title}/>
          ...
          <div
            className={`${this.bem("__out")} ${this.props.node.extras.type !== 'card' && this.props.node.extras.type}`}>
        ...
        <div id="add-command" onMouseUp={(e) => {
