...
const reorder = (list, newIndex, item) => {
  ...
  return [...remainingItems.slice(0, newIndex), item, ...remainingItems.slice(newIndex)];
};
export class DiagramWidget extends BaseWidget {
  ...
  getMouseElement(event) {
    ...
    if (element) {
      ...
      let port = diagramModel.getNode(nodeElement.getAttribute('data-nodeid')).getPort(element.getAttribute('data-name'));
      if (!port) {
        ...
        if (model.model instanceof NodeModel || (model.model instanceof PointModel && !model.model.isConnectedToPort())) {
          ...
          if (model.model instanceof NodeModel) {
            ...
            if (current && target_node && (Math.abs(amountX) > 5 || Math.abs(amountY) > 5)) {
              ...
              let overlapX =
                (current.x >= target_node.x && current.x <= target_node.x + target_node.width / amountZoom) ||
                (current.x + 220 >= target_node.x && current.x + 220 <= target_node.x + target_node.width / amountZoom);
              let overlapY =
                (current.y >= target_node.y && current.y <= target_node.y + target_node.height / amountZoom) ||
                (current.y + 35 >= target_node.y && current.y + 35 <= target_node.y + target_node.height / amountZoom);
              if (!overlapX || !overlapY) {
                  ...
                  _.map(current.parentCombine.getOutPorts(), (port) => {
                    if (!_.isEmpty(port.links) && port instanceof PortModel) {
                      ...
                      if (point instanceof PointModel && current.parentCombine.ports[port.name]) {
                        current.parentCombine.ports[port.name].links[point.parent.id].points[pointIdx].updateLocation({
                          x: point.x,
                          y: point.y - 40,
                        });
                      }
                ...
                if (
                  nodeIdx >= 0 &&
                  current.extras.type !== 'exit' &&
                  current.extras.type !== 'choice' &&
                  current.extras.type !== 'stream' &&
                  current.extras.type !== 'random' &&
                  current.extras.type !== 'interaction' &&
                  current.extras.type !== 'if'
                ) {
                  if (nodeIdx !== 0) {
                  ...
                  if (nodeIdx !== target_node.combines.length - 1) {
                    ...
                    if (
                      ...
                      nextNode !== 'temp' &&
                      (nextNode.extras.type !== 'choice' &&
                        nextNode.extras.type !== 'exit' &&
                        nextNode.extras.type !== 'stream' &&
                        nextNode.extras.type !== 'random' &&
                        nextNode.extras.type !== 'interaction' &&
                        nextNode.extras.type !== 'if')
                    ) {
  ...
  onKeyUp(event) {
    ...
    if (this.props.deleteKeys.indexOf(event.keyCode) !== -1) {
      ...
      if (
        ...
        first.extras &&
        first.combines &&
        first.combines.length !== 0 &&
        super_select &&
        super_select.parentCombine &&
        super_select.extras &&
        super_select.combines &&
        super_select.combines !== 0
      ) {
          ...
          if (!element.isLocked()) {
            ...
            if (element.extras && (element.extras.type === 'comment' || element.extras.type === 'story')) {
              ...
              this.props.onConfirm({
                ...
                text: (
                  <Alert color="danger" className="mb-0">
                    WARNING: This action can not be undone, <i>{element.name}</i> can not be recovered
                  </Alert>
                ),
              ...
              if (
                ...
                (element.parent.sourcePort.parent && (element.parent.sourcePort.parent.extras.type !== 'story' || element.parent.points.length > 2))
              ) {
                element.remove();
              }
            }
        ...
        _.forEach(this.state.action.selectionModels, (model) => {
          ...
          if (sourcePort !== null && targetPort !== null) {
            if (!sourcePort.canLinkToPort(targetPort)) {
              ...
              _.some(_.values(targetPort.getLinks()), (l) => l !== link && (l.getSourcePort() === sourcePort || l.getTargetPort() === sourcePort))
            ) {
              //link is a duplicate
              link.remove();
            }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <NodeLayerWidget diagramEngine={diagramEngine} nodeProps={this.props.nodeProps} />
