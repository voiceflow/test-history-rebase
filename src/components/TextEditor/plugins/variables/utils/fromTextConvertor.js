...
export const matchVariables = (text = '', { withoutTrim } = {}) => {
  ...
  const textWithFormattedVars = withoutTrim ? transformVariablesFromReadableWithoutTrim(text) : transformVariablesFromReadable(text);
...
const fromTextConvertor = () => ({ variables = [] } = {}) => (next) => (value, { cursor, entityMap, entityRanges, withoutTrim }) => {
