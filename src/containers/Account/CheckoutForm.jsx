...
class CheckoutForm extends React.Component {
  constructor(props) {
    ...
    const admin = window.user_detail.admin === 0 ? 0 : -1;
      ...
      .then(() => {
        ...
        if (source) {
          ...
          const pollForSourceStatus = () => {
            that.props.stripe.retrieveSource({ id: source.id, client_secret: source.client_secret }).then((result) => {
  ...
  renderForm() {
    const price = (
      <div className="price">
        <span className="text-pricing">${this.state.price || this.props.plan.price}</span>/{this.state.period || 'mo'}
      </div>
    );
    ...
    if (this.state.admin > 0) {
      ...
      return (
        <div className="pt-2 text-center">
          <Alert color="warning">This plan is not available</Alert>
        </div>
      );
    }
          ...
          {this.state.promo !== null && (
            <React.Fragment>
              ...
              {!!this.state.promo_invalid && (
                <div className="invalid-feedback">{this.state.promo_invalid === 2 ? 'Checking Promo Code...' : 'Invalid Promotional Code'}</div>
              )}
            </React.Fragment>
          )}
  ...
  render() {
    ...
    if (this.state.stage === 4) {
      return (
        <div className="payment-form pt-3">
          ...
          <Alert color="success">Please Log out and Sign In again</Alert>
          <Button onClick={this.props.logout} block>
            Logout
          </Button>
        </div>
      );
    ...
    if (this.state.admin === this.props.selected) {
      return (
        <div className="payment-form pt-2 text-center">
          <Alert>Subscribed to this plan</Alert>
          <img className="w-75 mt-3" src="/business.svg" alt="plan" />
        </div>
      );
    }
    ...
    return (
      <div className="payment-form">
        {this.renderStage()}
        <div className={cn({ 'd-none': this.state.stage !== 0 })}>{this.renderForm()}</div>
      </div>
    );
  }
}
