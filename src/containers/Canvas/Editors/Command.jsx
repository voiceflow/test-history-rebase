...
export class Command extends Component {
  ...
  static getDerivedStateFromProps(props) {
    ...
    if (!extras) {
      ...
      const intent = _.find(props.intents, { key: extras.intent.key });
      if (intent) {
        extras.intent = {
          label: intent.name,
          value: intent.key,
      ...
      if (Array.isArray(extras.mappings)) {
        ...
        extras.mappings.forEach((mapping) => {
          if (mapping.slot && mapping.slot.key) {
            const slot = _.find(props.slots, { key: mapping.slot.key });
            if (slot) {
  ...
  command(intent_options) {
    ...
    if (extras.resume) {
      ...
      if (extras.diagram_id) {
        const find = this.props.diagrams.find((d) => d.id === extras.diagram_id);
        if (find) {
          ...
          {extras.diagram_id ? (
            ...
            <React.Fragment>
              ...
              <DefaultButton
                ...
                isLarge
                isBlock
                onClick={() => this.props.createDiagram(this.state.node, this.state.node.name ? this.state.node.name : 'Command Flow', null, true)}
              >
              ...
              {this.props.diagrams && this.props.diagrams.length > 0 ? (
                <React.Fragment>
                  ...
                  <Select
                    ...
                    classNamePrefix="select-box"
                    onChange={(selected) => {
                      const diagram_id = selected.value.substring(0, selected.value.indexOf('::'));
                      PLATFORMS.forEach((p) => {
                        const extras = this.state.node.extras[p];
  ...
  renderTab() {
    const options = this.props.intents
      .concat(this.props.built_ins)
      .filter((intent) => {
        if (
          (intent._platform === 'google' && this.props.platform !== 'google') ||
          (intent._platform === 'alexa' && this.props.platform !== 'alexa')
        ) {
          return null;
        }
        return intent;
      ...
      .map((intent) => {
        const split = intent.name.split('.');
        const label = split[split.length - 1];

        return { label, value: intent.name, key: intent.key, inputs: intent.inputs, built_in: intent.built_in };
      });
