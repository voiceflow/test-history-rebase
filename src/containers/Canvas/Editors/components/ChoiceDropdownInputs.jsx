...
class ChoiceDropdownInputs extends Component {
  ...
  static getDerivedStateFromProps(props) {
    const new_choices = props.choices.map((choice_obj) => {
      ...
      const intent = _.find(props.intents, { key });
  ...
  render() {
    ...
    return (
      <div className="w-100">
        {Array.isArray(this.state.choices)
              ...
              return (
                <div className="interaction-block mb-3" key={choice.key}>
                  <div className="interaction-title">
                    ...
                    >
                      {choice.open ? <i className="fas fa-caret-down" /> : <i className="fas fa-caret-right" />} {i + 1}
                    </span>
          ...
          : null}
        <div>
          <Button isBtn isClear isLarge isBlock onClick={this.props.onAdd} disabled={this.props.live_mode}>
            <i className="far fa-plus mr-2" /> Add Choice
          </Button>
        </div>
      </div>
    );
