...
class ChoiceInput extends Component {
  constructor(props) {
    ...
    this.state = {
      samples: this.props.input
        .split('\n')
        .filter((e) => e.trim())
        .map((t) => ({ text: t, key: randomstring.generate(5) })),
      text: '',
    };
  ...
  shouldComponentUpdate(props, state) {
    ...
    if (this.state.text_error !== state.text_error) {
      return true;
    }
    return false;
  }
  ...
  renderUtterances = (utterances) => {
    if (Array.isArray(utterances)) {
      return utterances.map((u, i) => {
        ...
        return (
          <div className="choice-utterance" key={utterances[index].key}>
            <ContainedTextarea value={utterances[index].text} index={index} onChange={this.updateSample} />
            <i
              onClick={(e) => {
