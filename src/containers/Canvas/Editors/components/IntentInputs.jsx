...
class IntentInputs extends Component {
  ...
  handleRemoveIntent = (key) => {
    this.props.setConfirm({
      text: <Alert color="warning" className="mb-0">Make sure this Intent isn't used in any Command or Intent
        blocks<br/>-<br/>Deleting may cause unexpected behavior</Alert>,
      confirm: () => {
  ...
  render() {
    ...
    return (
      <div className="w-100">
        {length > 4 && <Input type="search" onChange={this.onSearchChange} id="searchIntents"
                              className="form-control-border mb-3 search-input" placeholder="Search Intents"
                              value={this.state.search_value}/>}
        {length < 251 && <button className="btn btn-clear btn-lg btn-block mb-3" onClick={this.handleAddIntent}
                                 disabled={this.props.live_mode}><i className="far fa-plus mr-2"/> Add Intent
        </button>}
        {reverse}
      </div>
    );
