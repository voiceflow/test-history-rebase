...
class Permission extends Component {
  ...
  render() {
    const variable_map = this.props.selected && PERMISSIONS_WITH_VARIABLE_MAPS.includes(this.props.selected.label);
    const VariableLabel = () => {
      if (variable_map) {
      ...
      if (this.props.selected.label === 'Product') {
        let consumable;
        if (this.props.permission && this.props.permission.product) {
          const product = this.props.products.find((p) => p.id === this.props.permission.product.value);
          if (_.get(product, ['data', 'type']) === 'CONSUMABLE') {
            consumable = true;
          }
        }
      ...
      <div className="solid-border set-block">
        ...
        <Select
          ...
          isOptionDisabled={(option) => {
            return _.find(this.props.disabled_perms, { selected: option }) && option.label !== 'Product';
          }}
        />
        <VariableLabel />
      </div>
