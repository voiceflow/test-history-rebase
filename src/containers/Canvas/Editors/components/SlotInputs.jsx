...
class SlotInputs extends Component {
  ...
  handleAddSlot() {
    let name = `slot_${getIndex(this.props.slots.length + 1)}`;
    ...
    this.props.slots.push({ name, inputs: [], type: '', key: randomstring.generate(12), open: true });
    this.props.update();
  ...
  handleRemoveSlot(key) {
    ...
    if (used_slots.has(key)) {
      ...
      const i = this.props.slots.findIndex((i) => i.key === key);
      if (i !== -1) {
  ...
  checkNames(name) {
    return this.props.slots.some((i) => i.name === name);
  }
  ...
  render() {
    ...
    if (Array.isArray(this.props.slots)) {
      ...
      for (i = this.props.slots.length - 1; i >= 0; i--) {
        ...
        if (slot.name.includes(this.state.search_value)) {
          reverse.push(
            <SlotInput
              key={slot.key}
              slot={slot}
    ...
    return (
      <div className="w-100">
        ...
        {length < 251 && (
          <button className="btn btn-lg btn-clear btn-block mb-3" onClick={this.handleAddSlot} disabled={this.props.live_mode}>
            <i className="far fa-plus mr-2" /> Add Slot
          </button>
        )}
        {reverse}
      </div>
