...
class ActionSection extends Component {
  ...
  selectAction(action) {
    ...
    if (!integration_data.actions_data) {
      this.props.updateIntegrationData(
        update(this.props.integration_data, {
          actions_data: {
            $set: {
  ...
  checkCompletion() {
    const completed = !!this.props.selected_action;
    this.setState({
      completed,
    });
  }
            ...
            {Object.keys(this.props.all_actions).map((e, i) => {
              return (
                <div
                  ...
                  onClick={() => this.selectAction(e)}
                  style={{ position: 'relative' }}
                >
                  <Tooltip className="menu-tip actions-tooltip" title={this.props.all_actions[e].tooltip} position="bottom" theme="block">
                    ?
                  </Tooltip>
                  {e}
                </div>
