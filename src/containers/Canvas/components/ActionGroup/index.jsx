...
const invNameError = (name, locales) => {
	...
	let inv_name_error = `[${locales.filter(l => l !== 'jp-JP')
	...
	if (locales.length === 1 && locales[0] === 'ja-JP') {
		...
		inv_name_error = `[${locales.filter(l => l.includes('en'))
	...
	} else if (WAKE_WORDS.some(l => split_name.find(split => {
		return split === l.toLowerCase();
	}))) {
		return 'Invocation name cannot contain Alexa keywords e.g. ' + WAKE_WORDS.join(', ');
	} else if (LAUNCH_PHRASES.some(l => split_name.find(split => {
		return split === l.toLowerCase();
	}))) {
...
export class ActionGroup extends PureComponent {
	...
	updateGoogle = () => {
		...
		axios.post(`/project/${this.props.skill.project_id}/render`, {
			platform: 'google',
	...
	togglePreview = () => {
		...
		this.props.togglePreview(!this.props.skill.preview)
			.then(() => this.setState({ togglingPreview: false }));
	};
	...
	renderLiveStage = () => {
		if (this.state.live_update_stage === 2) {
			...
			return <React.Fragment>
				...
				<div className="modal-txt text-center mt-2 mb-3">This update will affect the live version of your project. Please be sure you wish to
					do this.
				</div>
		...
		switch (this.state.stage) {
			...
			case 2:
				...
				if (!modal) {
					return <div className="text-center">
						<div className="d-flex align-items-center justify-content-center upload-prompt-title mb-2"><span
							className="pass-icon mr-2"/> Upload Successful
						</div>
						<div className="upload-prompt-text">
							Your Skill is now available to test on your Alexa and the <a
			...
			case 6:
				return <div className={'w-100 ' + (modal ? 'text-center' : '')}>
					...
					<div className="text-muted mb-4 margin-auto" style={{ maxWidth: 350 }}><b>Important:</b> Make sure to use the same email
						associated with your Amazon account.
					</div>
					<hr className="full-width"/>
					<div className={modal ? 'super-center mb-2' : ''}>
						<a href="https://developer.amazon.com/login.html" className="btn-primary mr-3 no-underline d-inline-block mb-2"
						   target="_blank" rel="noopener noreferrer">
							Developer Sign Up
						</a>
			...
			case 14:
				return <div className="w-100">
					<div className="d-flex text-muted align-items-center">
						...
						<Tooltip
							html={(<React.Fragment>Alexa listens for the Invocation Name<br/> to launch your Skill<br/> e.g. <i>Alexa, open <b>Invocation
								Name</b></i></React.Fragment>)}
							position="bottom"
						>
							<i className="fal fa-question-circle"/>
						</Tooltip>
					</div>
					<input className="form-control" value={this.state.inv_name} placeholder='Invocation Name' onChange={(e) => this.setState({
			...
			default:
					...
					return <div id="name-box" className="text-center">
						...
						<div className="grid-col-3 mx--1">
							{LOCALE_MAP.map((locale, i) => {
								...
								return <Button className={`country-checkbox btn-darken ${active}`} key={i} onClick={() => {
						...
						<div className="mt-4 mb-5">
							<Button isPrimary varient="contained" onClick={(e) => {
								this.updateInvName();
								this.updateAlexa();
				...
				return <div>
					...
					<div className="modal-txt text-center mt-2"> Updating to Alexa will allow you to test on your Alexa device or the Alexa Developer
						Console
					</div>
		...
		if (
			...
			if (!modal) {
				modal_content = <div className="text-center">
					...
					<div className="upload-prompt-text">
						You may test on the <a
						href={`https://console.actions.google.com/u/${this.props.skill.google_publish_info.google_link_user || '0'}/project/${this.state.google_id}/simulator`}
						target="_blank" rel="noopener noreferrer">
			...
			modal_content = <div>
				...
				<div className="modal-txt text-center mt-2"> Updating to Google will allow you to test on your Google device or the Google Actions
					Console.
				</div>
	...
	render() {
		...
		return (
			<React.Fragment>
				...
				<Modal size={this.state.stage === 0 ? 'lg' : undefined} isOpen={this.state.updateModal && this.state.is_first_upload}
				       toggle={() => this.setState({ updateModal: false })} onClosed={this.shouldReset} className="stage_modal">
				...
				<Modal isOpen={this.state.updateLiveModal} toggle={this.toggleUpdateLive} onClosed={() => {
          ...
          preview={this.props.preview}
					...
					leftRenderer={() => (
						<div onDoubleClick={() => this.setState({ editName: true })}>
							...
							{this.state.editName ? <input autoFocus className="edit-input" value={this.props.skill.name} onChange={e => {
									this.props.updateSkill('name', e.target.value);
									this.props.updateSkill('inv_name', e.target.value);
                ...
                <div id="middle-group">
                  ...
                  >
                    ...
                    }} type="radio" className={`switch-input ${this.props.platform === 'alexa' ? 'checked' : ''}`} value="alexa_toggle"
                           id="alexa_toggle"/>
                    ...
                    }} type="radio" className={`switch-input ${this.props.platform === 'google' ? 'checked' : ''}`} value="google_toggle"
                           id="google_toggle"/>
