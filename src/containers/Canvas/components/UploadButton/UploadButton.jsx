	...
	render() {
		...
		if (this.props.live_mode) {
			...
			if (this.props.isUploadLoading()) {
				return <Button variant="contained" className="publish-btn"
				               onClick={() => this.props.toggle_upload_prompt()}>
				...
				return <div className={cn('upload-btn-container', { 'multi-vendor': multiVendor })}>
					<Tooltip
						html={<div
							style={{ width: 180 }}>{(this.props.platform === 'google') ? 'Test your Action on your own Google device, or in the Google Actions console' : 'Test your Skill on your own Alexa device, or in the Alexa developer console'}</div>}
						position="bottom"
						distance={16}
