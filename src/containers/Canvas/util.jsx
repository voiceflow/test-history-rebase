  ...
  for (i = diagram.nodes.length - 1; i >= 0; i -= 1) {
    ...
    if (node.extras) {
      ...
      if (node.extras.type === 'flow' && node.extras.diagram_id) {
        const find = diagrams.find((x) => x.id === node.extras.diagram_id);
        if (find) {
          node.name = find.name;
        }
    ...
    if (Array.isArray(node.ports)) {
      ...
      if (!_.isEmpty(node.combines)) {
        node.combines = _.without(node.combines, (combine) => combine === 'temp');
      ...
      ports: [
        {
          ...
          parentNode: '88888888-8888-8888-8888-888888888888',
          links: [],
          in: false,
          label: ' ',
        },
...
const serializeDiagram = (engine) => {
  const serialize = engine.getDiagramModel().serializeDiagram();
  serialize.nodes.forEach((node) => {
    if (!_.isEmpty(node.combines)) {
      const isHome = node.extras.type === 'story';
      ...
      node.combines = _.map(node.combines, (combine, idx) => {
        ...
        if (!isHome && idx !== node.combines.length - 1 && combine.extras) {
...
const createDropNode = (event, engine, type, name) => {
  ...
  if (type) {
    if (type === 'choice') {
      ...
      const diagram_id = event.dataTransfer && event.dataTransfer.getData('diagram_id');
      node.addInPort(' ');
      node.addOutPort(' ').setMaximumLinks(1);
      node.extras = {
        diagram_id,
      ...
      const data = event.dataTransfer && event.dataTransfer.getData('data');
      node.addInPort(' ');
      node.addOutPort(' ').setMaximumLinks(1);
      node.addOutPort('fail').setMaximumLinks(1);
      node.extras = {
      ...
      try {
        ...
        const inputs = data.input ? JSON.parse(data.input) : [];
        const outputs = data.output ? JSON.parse(data.output) : [];
        ...
        node.extras = {
          ...
          mapping: {
            inputs: inputs.map(buildKeyValue),
            outputs: outputs.map(buildKeyValue),
          },
...
export { canSave, generateID, serializeDiagram, convertDiagram, createDropNode, createCombineNode };
