...
export const vendorByIDSelector = createSelector([activeMemberSelector], (member) => (vendorID: string) =>
  member?.platformData.vendors.find((vendor) => vendor.vendorID === vendorID) ?? null
);
...
export const updateActiveVendor = (vendorID: string, skillID: string | null): SyncThunk => (dispatch, getState) => {
