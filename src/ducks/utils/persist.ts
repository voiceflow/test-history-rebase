 ...
 */
const createPersistor = (store: Storage) => <T extends NotUndefined = never>(
  stateKey: string,
  key: string
 ...
 */
export const rehydrateReducer = <T extends NotUndefined>(persistor: Persistor<T>, initialValue: T): RootReducer<T, any> => (state) => {
  if (state !== undefined) {
    return state;
  }
