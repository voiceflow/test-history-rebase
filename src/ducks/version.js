...
export const updateIntents = () => {
  return (dispatch, getState) => {
    ...
    intents.forEach((intent, i) => {
      ...
      const intent_slots = getSlotsForKeys(intent.inputs.map((input) => input.slots), slots);
...
export const setCanFulfill = (intent_key, new_value) => {
  return (dispatch, getState) => {
    ...
    const fulfillment = fulfillments[intent_key];
...
export const syncInvocationName = (name) => async (dispatch, getState) => {
  ...
  try {
    await axios.patch(`/skill/${skills.skill.skill_id}?inv_name=1`, { inv_name: name });
  ...
  return (dispatch, getState) => {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const global_variables = ['sessions', 'user_id', 'timestamp', 'platform', 'locale'];
