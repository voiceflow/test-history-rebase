...
export const saveSettings = (settings: Partial<GoogleVersionSettings>): Thunk => async (dispatch, getState) => {
...
export const savePublishing = (publishing: Partial<GoogleVersionPublishing>): Thunk => async (dispatch, getState) => {
  const state = getState();
  const versionID = Session.activeVersionIDSelector(state);
