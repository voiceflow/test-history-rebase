...
export const loadVersionByID = (versionID: string): Thunk<AnyVersion> => async (dispatch, getState) => {
...
export const activateVersion = (versionID: string, diagramID?: string): Thunk<AnyVersion> => async (dispatch, getState) => {
  const state = getState();
  const platform = Project.activePlatformSelector(state);
...
export const saveLocales = (locales: AnyLocale[]): Thunk => async (dispatch, getState) => {
...
export const saveSession = (session: Partial<Models.Version.Session>): Thunk => async (dispatch, getState) => {
  const state = getState();
  const versionID = Session.activeVersionIDSelector(state);
...
export const saveResumePrompt = (resumePrompt: Partial<Models.Version.Session['resumePrompt']>): Thunk => async (dispatch, getState) => {
  const state = getState();
  const activeSession = activeSessionSelector(state);
