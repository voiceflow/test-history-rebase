...
export const removeGlobalVariable = (variable: string): SyncThunk => (dispatch, getState) => {
  const state = getState();
  const versionID = Session.activeVersionIDSelector(state);
  const variables = activeGlobalVariablesSelector(state);
...
export const replaceGlobalVariables = (variables: string[], meta?: object): SyncThunk => (dispatch, getState) => {
  const state = getState();
  const versionID = Session.activeVersionIDSelector(state);
