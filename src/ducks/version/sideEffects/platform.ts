...
export const updateLocalesByVersionID = <L extends AnyLocale>(versionID: string, locales: L[]): SyncThunk => (dispatch, getState) => {
...
export const patchActiveVersion = (version: Partial<Pick<AnyVersion, 'session' | 'settings' | 'publishing'>>, meta?: object): SyncThunk => async (
  dispatch,
  getState
) => {
  const state = getState();
...
export const saveSettings = (settings: Partial<AnySettings>): Thunk => async (dispatch, getState) => {
  const state = getState();
