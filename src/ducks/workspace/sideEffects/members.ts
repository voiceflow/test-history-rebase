...
export const loadMembers = (workspaceID: string): Thunk => async (dispatch) => {
  try {
...
export const sendInviteToActiveWorkspace = (email: string, permissionType: UserRole, showToast = true): Thunk<boolean> => async (
  dispatch,
  getState
) => {
...
export const updateInviteToActiveWorkspace = (email: string, permissionType: UserRole): Thunk => async (dispatch, getState) => {
  const state = getState();
  const activeWorkspaceID = Session.activeWorkspaceIDSelector(state);
  const currentMembers = activeWorkspaceMembersSelector(state);
...
export const cancelInviteToActiveWorkspace = (email: string): Thunk => async (dispatch, getState) => {
  const state = getState();
  const activeWorkspaceID = Session.activeWorkspaceIDSelector(state);
  const currentMembers = activeWorkspaceMembersSelector(state);
...
export const updateMemberOfActiveWorkspace = (creatorID: number, role: UserRole): Thunk => async (dispatch, getState) => {
  const state = getState();
  const activeWorkspaceID = Session.activeWorkspaceIDSelector(state);
  const currentMembers = activeWorkspaceMembersSelector(state);
...
export const deleteMemberOfActiveWorkspace = (creatorID: number): Thunk => async (dispatch, getState) => {
  const state = getState();
  const activeWorkspaceID = Session.activeWorkspaceIDSelector(state);
  const currentMembers = activeWorkspaceMembersSelector(state);
...
export const updateActiveWorkspaceMemberRole = (member: Workspace.Member, role: UserRole): Thunk => async (dispatch, getState) => {
  const state = getState();
