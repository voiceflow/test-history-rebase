...
const VisualEditor: NodeEditor<NodeData.Visual> = ({ data, onChange }) => {
  ...
  const onBlurDimensions = () => {
    onChange({
      dimensions: {
        width: Number(dimensions?.width ?? '0') || data.dimensions?.width || 500,
        height: Number(dimensions?.height ?? '0') || data.dimensions?.height || 500,
      },
    });
  };
