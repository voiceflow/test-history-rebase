...
export type TraceControllerProps = {
  ...
  updateStatus: (status: TMStatus) => void;
...
class TraceController {
  ...
  public next = async (request?: any) => {
    this.props.updateStatus(TMStatus.FETCHING_CONTEXT);
  ...
  private async processTrace(trace: Trace[], { onlyMessage = false }: { onlyMessage?: boolean } = {}) {
    ...
    if (!topTrace) {
      this.props.updateStatus(TMStatus.WAITING_USER_INTERACTION);
      return;
    }

    this.trace = tailTrace;

    this.props.updateStatus(TMStatus.NAVIGATING);
  ...
  private async processEndTrace(topTrace: EndTrace) {
    ...
    this.props.updateStatus(TMStatus.ENDED);
  }

  private setError = (message = 'Unable to play an audio') => {
    this.props.updateStatus(TMStatus.ERROR);
