          ...
          {Object.keys(INTEGRATIONS).map((k) => {
            ...
            return <div key={k} className='wrapper'>
              <div className={`item d-flex flex-column align-items-center ${integrationsInfo.component ? '' : 'faded'}`} onClick={() => {
                ...
                {integrationsInfo.component ? <Tooltip
                  className="menu-tip integrations-tooltip"
                  title={integrationsInfo.tooltip}
                  position="bottom"
                  theme="block"
                >
                  ?
              </Tooltip> : <div className="soon-badge">Soon</div>}
              </div>
            </div>
          })}
        </div>
    ...
    if (!extras.selected_integration) {
      ...
      view = (
        <div className='d-flex align-items-center flex-column integrations-container'>
          <div className='actions-back back-btn-large btn-icon' onClick={() => this.deselectIntegration()} ></div>
          <div><img className='title-image' src={integrationInfo.image} alt="empty" /></div>
          <hr className='mb-0' />
