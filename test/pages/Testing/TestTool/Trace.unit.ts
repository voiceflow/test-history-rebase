...
import TraceController from '@/pages/TestingV2/TestTool/Trace';
import { TMStatus } from '@/pages/TestingV2/types';
...
const suite = createSuite(({ spy, stub, mock, expect }) => ({
  ...
  expectSetError(controller: TraceController) {
    expect(controller['props']['updateStatus']).to.be.calledWith(TMStatus.ERROR);
  ...
  expectProcessSingleTrace(controller: TraceController, methodName?: TraceMethods, { onlyMessage = false }: { onlyMessage?: boolean } = {}) {
    ...
    expect(controller['props']['updateStatus']).to.be.calledWith(TMStatus.NAVIGATING);
    expect(controller['props']['updateStatus']).to.be.calledWith(TMStatus.WAITING_USER_INTERACTION);
  ...
  ({
    ...
    describe('next()', () => {
      it('should change the status', async () => {
        ...
        expectUpdateStatus(controller).to.be.calledWith(TMStatus.FETCHING_CONTEXT);
      });
      ...
      it('should process speak trace audio error', async () => {
        ...
        expectUpdateStatus(controller).to.be.calledWith(TMStatus.ERROR);
      ...
      it('should process stream play', async () => {
        ...
        expectUpdateStatus(controller).to.be.calledWith(TMStatus.WAITING_USER_INTERACTION);
        expectAudioPlay(controller).to.be.calledWithMatch(SRC);
      ...
      it('should process end trace', async () => {
        ...
        expectUpdateStatus(controller).to.be.calledWith(TMStatus.ENDED);
      });
      ...
      it('should not process traces if stopped', async () => {
        ...
        expectUpdateStatus(controller).not.to.be.calledWith(TMStatus.NAVIGATING);
      });
    });
